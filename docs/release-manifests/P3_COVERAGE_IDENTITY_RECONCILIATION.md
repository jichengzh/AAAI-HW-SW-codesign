...
因此，截至 P3-42，P3 的真实唯一覆盖为 **278/1,551**。P3-43--P3-54 分别从当时的唯一身份集合之外各新增 12 个身份，当前真实进度为 **422/1,551**。P3-50 入账前重建验证了全部 373 条此前 accepted records：每条均与冻结 P2 metadata、当前 SHA-256 和规范 HMAC ledger 对齐，去重结果仍为此前的 362 个身份；P3-50--P3-54 各再新增 12 个身份。后续批次必须从未出现在该集合中的候选确定性抽取，并在入队前再次运行本对账；任何相加计数都不得作为 P3 关闭依据。
