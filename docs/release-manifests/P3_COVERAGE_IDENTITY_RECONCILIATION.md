...
因此，截至 P3-42，P3 的真实唯一覆盖为 **278/1,551**。P3-43 已在入队前排除这 278 个身份，并经同一对账确认新增 12 个身份；P3-44 再从该 290 个身份之外新增 12 个身份；P3-45 从该 302 个身份之外新增 12 个身份。当前真实进度为 **314/1,551**。后续批次必须从未出现在该集合中的候选确定性抽取，并在入队前再次运行本对账；任何相加计数都不得作为 P3 关闭依据。
