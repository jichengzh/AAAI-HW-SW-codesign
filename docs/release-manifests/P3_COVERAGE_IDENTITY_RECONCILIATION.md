...
**当前更正（覆盖此前当前计数）**：P3-146 新增 12 个冻结 P2b 身份；受限 inventory、当前 SHA-256、第三方逐项裁决、HMAC ledger 与 acceptance manifest 均已重算。最终 P4=3、P6=5、P7=4，无重复项，真实唯一覆盖为 **1,520/1,551**，尚余 **31** 项。q2 的私有 `/tmp` 测量日志，以及 q7/q11/q12 的外部论文/训练知识来源与许可链未闭合，须清洗、核验并扫描后重审；P3 尚未关闭，P4 尚未开始。

**当前更正（覆盖此前当前计数）**：P3-147 新增 12 个冻结 P2b 身份；受限 inventory、当前 SHA-256、双独立逐项审查、HMAC ledger 与 acceptance manifest 均已重算。最终 P4=9、P6=2、P7=1，无重复项，真实唯一覆盖为 **1,532/1,551**，尚余 **19** 项。q10 的 D-space 文档含 `/tmp/nvidia-mps` 私有临时路径，须参数化、清洗并扫描后重审；P3 尚未关闭，P4 尚未开始。
