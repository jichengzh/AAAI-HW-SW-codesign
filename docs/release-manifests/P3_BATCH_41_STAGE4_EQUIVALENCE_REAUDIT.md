...
复审只读取冻结基线和公开替代实现；未执行私有模型、数据、设备任务或外部网络操作。受限原始记录不进入公开树；公开账本仅保留路径无关、可复算的 HMAC 承诺。全量 P3 records 后续经身份去重，确认 11 条为既有 P2 身份的复审；因此本批纳入后的真实唯一覆盖为 **278/1,551**，而不是 records 相加的 289。详见 [覆盖身份对账](P3_COVERAGE_IDENTITY_RECONCILIATION.md)。
