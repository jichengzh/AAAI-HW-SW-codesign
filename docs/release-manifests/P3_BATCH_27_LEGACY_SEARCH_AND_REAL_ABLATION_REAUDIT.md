...
状态：主审完成，待独立复审。本批已逐项读取 12 个候选的当前内容，核对其 P2 metadata/字节数、当前 SHA-256、职责、导入/调用、公开替代物、来源/许可证和安全输出。没有执行私有脚本、数据、模型、设备、子进程或生成物。独立复审返回前，本批不计入已接受 re-audit 总数。
...
本批不迁入源码、测试、数据、模型、checkpoint、ONNX、engine 或实验结果。受限 HMAC evidence/decision records 仅用于本机核验，不构成 P3 关闭账本。由于独立复审尚未回收，新的发布级 re-audit 仍保持 85/1,551；P4 尚未开始。
