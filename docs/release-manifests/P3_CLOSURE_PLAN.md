...
当前有九个已接受的局部批次：P3-18 的 8 个合成测试 fixture、P3-19 的 5 个外部边界候选、P3-20--P3-25 的六个 12 项批次，以及 P3-28 的 12 项，共 97/1,551 项。P3-25 中 4 项在入队快照后发生内容漂移，已按当前内容重新读取、重新绑定受限 SHA-256 证据后才计入；P3-28 当前内容漂移为 0 且独立样本复审通过。P3-26 和 P3-27 已完成主审且当前内容漂移为 0，但独立复审尚未回收，因此不计入已接受总数。所有受限 evidence/decision/HMAC ledger 检查点均不构成关闭账本；P4 尚未开始。
