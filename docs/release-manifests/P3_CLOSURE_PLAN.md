...
状态：候选审计与全量 ledger 已完成；完整可复现发布关闭仍未满足，不代表已完成公开发布。P3-17 已确认 1,551 项 decisions 与路径无关 ledger，但 P4--P7 的契约/许可工作仍是关闭前置条件。
...
每一次 P3 批次开始前必须用 v1.1.0 inventory 工具在受控临时目录重跑盘点；若基线哈希或计数改变，先记录增量并处置新增/删除项。原始逐路径报告、私有路径和其映射绝不提交。
...
每条公开记录还须绑定 P2 分类、批次、固定 reason code、许可结论、公开替代物/契约、测试或审阅证据及后续阶段。账本不得含 `pending` 或 `reviewing`；计数必须精确回填至 1,551。
...
当前不允许将远端称为完整公开发布版本：P3 的逐项处置账本已生成，但 P4--P8 仍未完成，且 P7 尚有许可证/隐私阻塞。P4--P7 实际完成后，才可以评估将通过同一 HEAD 验证的分支同步到已确认私有的协作远端；这只是阶段性备份，不是开源发布。任何 `git push`、可见性变更、tag 或 Release 都需要维护者对此次外部动作的明确授权。
