...
状态：进行中；本文件定义 P3 的关闭条件和当前缺口，不代表已完成公开发布。
...
当前不允许将远端称为完整公开发布版本：P3 的逐项处置账本尚未生成，且 P4--P8 未完成。P3 完成后，可以评估将通过当前 HEAD 验证的分支同步到已确认私有的协作远端；这只是阶段性备份，不是开源发布。任何 `git push`、可见性变更、tag 或 Release 都需要维护者对此次外部动作的明确授权。
