...
当前有二十一个已接受的局部批次：P3-18 的 8 个合成测试 fixture、P3-19 的 5 个外部边界候选、P3-20--P3-36 的十七个 12 项批次，以及 P3-38--P3-39 的两个 12 项批次，共 241/1,551 项。P3-25 中 4 项在入队快照后发生内容漂移，已按当前内容重新读取、重新绑定受限 SHA-256 证据后才计入；P3-26/P3-27 的冲突本地 records 已由保留旧记录的 supersession manifest 解决，唯一规范 records 已重新验证当前内容并重建账本；P3-28--P3-36 与 P3-38--P3-39 中已接受批次当前内容漂移均为 0，且均已有独立语义复审。P3-35 的前两次审阅有 8 个边界分歧，已由第三次逐项裁决、公开 Stage2 契约回归和重新计算账本解决；P3-36 的两项公开替代分歧也已由第三次职责/API/测试裁决解决；P3-38 的两个 P4/P6 finalizer 分歧已由逐项读取测试与被测入口的裁决解决。P3-37 仅完成主审，P3-40--P3-42 尚未完成独立复审，均不计入总数。所有受限 evidence/decision/HMAC ledger 检查点均不构成关闭账本；P4 尚未开始。
