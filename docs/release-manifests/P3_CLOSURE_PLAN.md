...
**最新更正（覆盖上段的当前计数）**：P3-96 完成 5 项 P4 外部制品契约和 7 项 P6 执行契约，并新增 12 个身份。主审与独立复审均先核验同一固定队列 SHA；q1、q2、q6、q7、q11、q12 经第三次只读裁决：rerun jobs 与 TensorRT build probe 归 P6，launch gate 与 scoped audit 归 P4。当前真实唯一覆盖为 **926/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-96 记录](P3_BATCH_96_STAGE6_EXECUTION_AND_EVIDENCE_REAUDIT.md)。
...
**最新更正（覆盖上段的当前计数）**：P3-99 完成 6 项 P4 外部制品契约、2 项 P6 执行契约和 4 项非必要排除，并新增 12 个身份。q1--q8 经第三次只读裁决：私有 Phase2/Stage6 验证测试因绑定 checkpoint、评测、报告、指标或其他外部制品而归 P4；直接覆盖实际 TVM measurement 或短时 finetune runner 的两项归 P6；没有测试可因未来迁入而提前记为 `rewritten_public`。同时拒绝了五项不合法 `excluded_nonessential → P7` 草案。当前真实唯一覆盖为 **962/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-99 记录](P3_BATCH_99_PHASE2_TEST_AND_DEBUG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-100 完成 5 项 P4 外部制品契约、1 项 P6 执行契约和 6 项非必要排除，并新增 12 个身份。第三方许可证与职责裁决确认两个带上游版权头的候选具有 Apache-2.0 来源依据，不应仅因版权头转 P7：日志分析归 P4，实际加载 config/checkpoint/dataset 并进行 GPU benchmark 的入口归 P6。其余为外部可视化制品或一次性调试表面；没有候选具备公开等价实现与测试，不能提前迁入。当前真实唯一覆盖为 **974/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-100 记录](P3_BATCH_100_ANALYSIS_AND_DEBUG_REAUDIT.md)。
...
**最新更正（覆盖上段的当前计数）**：P3-105 完成 12 项 `migrate_config` 候选的逐项复审，其中 1 项归 P5 环境契约、10 项归 P6 执行契约、1 项为非必要排除。全部 12 项的主审与独立复审发生边界分歧，均已由第三方逐项只读裁决：硬件能力/工具链描述归 P5，作为真实剪枝、量化或评测输入的主动采样配置归 P6，无支持链调用的内部审查收据可排除。当前真实唯一覆盖为 **1,030/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-105 记录](P3_BATCH_105_CONFIGURATION_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-106 完成 12 项 `migrate_config` 候选的逐项复审，全部归 P6 执行契约。主审与独立复审的全部边界分歧均由第三方逐项只读裁决：这些主动采样配置既有实际结构剪枝参数，也有量化路径的无剪枝控制面，均被真实剪枝、量化、评测或执行路由消费；配置是静态 JSON 或尚无公开等价物均不足以把其改记为 P4。当前真实唯一覆盖为 **1,042/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-106 记录](P3_BATCH_106_ACTIVE_CONFIG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-107 完成 12 项 `migrate_config` 候选的逐项复审，全部归 P6 执行契约。所有分歧均由第三方逐项只读裁决：这组剪枝配置直接进入实际结构变换、微调、量化、评测或执行路由；其配置文本与实验契约属性不改变其作为执行输入的主导职责，且缺少公开等价物不能自动转为 P4。当前真实唯一覆盖为 **1,054/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-107 记录](P3_BATCH_107_PRUNING_CONFIG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-108 完成 12 项 `migrate_config` 候选的逐项复审，全部归 P6 执行契约。全部边界分歧均由第三方逐项只读裁决：解耦、强度扫描和扩展剪枝预设直接控制模型加载后的结构剪枝、微调、量化、评测、测速或执行路由；其配置形式、实验矩阵用途或暂无公开等价物均不能自动转为 P4。当前真实唯一覆盖为 **1,066/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-108 记录](P3_BATCH_108_PRUNING_PRESET_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-109 完成 12 项 `migrate_config` 候选的逐项复审，全部归 P6 执行契约。全部边界分歧均由第三方逐项只读裁决：decoder 层数直接改变模型结构；量化配对配置即使选择 FP32/no-quant，仍作为控制面决定剪枝、量化或联合评测的实际执行路由。两类文件均不是纯发布契约，且没有公开等价物不能自动转为 P4。当前真实唯一覆盖为 **1,078/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-109 记录](P3_BATCH_109_DECODER_QUANT_CONFIG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-110 完成 12 项 `migrate_config` 候选的逐项复审，全部归 P6 执行契约。全部边界分歧均由第三方逐项只读裁决：FP32/no-quant 配对配置作为控制面决定实际剪枝或量化执行分支；INT8/混合精度配置直接控制量化模型变换和评测路径。它们都是执行输入，静态 JSON、实验矩阵用途或没有公开等价物均不能自动转为 P4。当前真实唯一覆盖为 **1,090/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-110 记录](P3_BATCH_110_ACTIVE_QUANT_CONFIG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-111 完成 12 项 `migrate_config` 候选的逐项复审，其中 11 项归 P6 执行契约、1 项归 P4 外部制品契约。全部边界分歧均由第三方逐项只读裁决：主动或默认量化配置直接决定 Q/DQ、模型变换、量化评测或执行路由；大规模逐层校准尺度制品则以来源、可公开性和可再生性为主导依赖。静态审阅未发现须升级为 P7 的许可或隐私阻塞，且配置形式、实验矩阵用途或无公开等价物均不能自动转为 P4。当前真实唯一覆盖为 **1,102/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-111 记录](P3_BATCH_111_QUANT_ARTIFACT_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-112 完成冻结基线最后 10 项 `migrate_config` 候选的逐项复审：数量自然不足 12，未补齐或重复；其中 4 项量化执行配置归 P6、6 项数据转换元数据/标签更新制品归 P4。全部分歧均由第三方逐项只读裁决：同字节私有镜像但没有公开实现与测试/API职责证据，不能提前记为迁入或重复；静态审阅未见明确许可或隐私阻塞，不能仅因数据制品体积或内容类型升级为 P7。`migrate_config` 现已完全审阅；代码、fixture 与配置共 1,112 项已处置，后续仅审阅 439 项 `migrate_document` 候选。当前真实唯一覆盖为 **1,112/1,551**；P3 仍在进行中，P4 尚未开始。详见 [P3-112 记录](P3_BATCH_112_FINAL_CONFIG_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-113 完成 12 项 `migrate_document` 候选的逐项复审，最终为 2 项 P4、2 项 P5、6 项 P6、1 项 P7 和 1 项非必要排除。4 项职责分歧均由第三方逐项只读裁决：量化/导出/执行计划和进展文档归 P6，资产与复现边界归 P4，环境与脱敏远程访问规范归 P5，瞬态机器状态交接因无当前公开支持链且维护性职责已由既有公开发布状态与复现边界材料承担而排除。一个测试规划文档因确认含未脱敏私有路径引用而保持 P7 阻塞；公开记录不披露该路径。本地镜像均不构成公开等价物。当前真实唯一覆盖为 **1,124/1,551**，剩余 427 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-113 记录](P3_BATCH_113_DOCUMENT_BOUNDARY_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-114 完成 12 项 `migrate_document` 候选的逐项复审，最终为 5 项 P4、3 项 P6 和 4 项 P7。4 项职责分歧均由第三方逐项只读裁决：已结束量化实验的状态记录和论文图表计划以外部制品为主导，归 P4；Q/DQ 修复路线和会启动/编排 GPU lane 的 supervisor 计划归 P6；同主题公开模块、CLI 和测试未承担同职责的论文图表源数据/导出契约，不能提前记为重复或已迁入。4 份文档确认含未脱敏私有路径引用或外部执行引用而保持 P7；公开记录不披露这些引用。当前真实唯一覆盖为 **1,136/1,551**，剩余 415 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-114 记录](P3_BATCH_114_DOCUMENT_EXECUTION_EVIDENCE_SECURITY_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-115 完成 12 项 `migrate_document` 候选的逐项复审，最终为 7 项 P4、1 项 P6、1 项 P7、1 项已安全公开改写、1 项公开实现已替代和 1 项非必要排除。9 项职责分歧均由第三方逐项只读裁决：论文/实验制品、研究资料索引和历史研究证据归 P4；含 HIL 调度、服务、trace 与控制器编排的设计归 P6；含私有远程访问参数、口令式认证流程和受控执行引用的运行入口保持 P7，公开记录不披露这些信息。只有经公开 fixture 测试或公开 Stage1→搜索空间 API/test 支持的文档才关闭为已改写或已替代；归档目录本身只有在无当前公开支持链、且其维护性职责已由既有公开发布、复现材料和测试承担时才可排除，实际承载实验/论文证据的归档文档不得仅因归档而排除。当前真实唯一覆盖为 **1,148/1,551**，剩余 403 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-115 记录](P3_BATCH_115_RESEARCH_DESIGN_ARCHIVE_AND_ACCESS_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-116 完成 12 项 `migrate_document` 候选的逐项复审，最终为 8 项 P4、2 项 P5、1 项 P6 和 1 项 P7。6 项职责分歧均由第三方逐项只读裁决：硬件文献与各类实验/论文结果边界归 P4，平台能力、工具链和 TVM 硬件 gate 归 P5，实际模型/编译/评测流水线计划归 P6；泛化硬件路径或结果文件名不构成 P7，但确认含未脱敏私有路径引用的多模型探测计划保持 P7，公开记录不披露该引用。当前真实唯一覆盖为 **1,160/1,551**，剩余 391 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-116 记录](P3_BATCH_116_HARDWARE_AND_METHOD_DESIGN_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-117 完成 12 项 `migrate_document` 候选的逐项复审，最终为 6 项 P4、3 项 P6 和 3 项 P7。9 项职责分歧均由第三方逐项只读裁决：受控测量、论文证据、制品登记和设计口径归 P4，实际 TVM 调优、LUT 生成与 worker/验证交接归 P6；真实私有路径、进程标识或远程连接/认证流程构成 P7 阻塞，公开记录不披露这些信息。设计文本或泛化硬件词本身不自动升级为 P6/P7；历史 inventory 和进展记录也不因含后续使用说明而失去其证据边界。当前真实唯一覆盖为 **1,172/1,551**，剩余 379 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-117 记录](P3_BATCH_117_AUTOTUNING_EVIDENCE_AND_ACCESS_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-118 完成 12 项 `migrate_document` 候选的逐项复审，最终为 1 项 P5、6 项 P6 和 5 项 P7。6 项职责分歧均由第三方逐项只读裁决：AP replay/微调、评估、三臂 readiness 与生产门禁归 P6，纯 AP 训练环境阻塞归 P5；泛化 PID 写法不单独构成 P7，但明确远程登录、临时认证和访问命令构成 P7，即使结果或制品主体将来可在拆分后转为 P4/P5。公开记录不披露这些流程。当前真实唯一覆盖为 **1,184/1,551**，剩余 367 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-118 记录](P3_BATCH_118_AP_STABILITY_AND_PRODUCTION_ACCESS_REAUDIT.md)。
...
**最新更正（覆盖上段的当前计数）**：P3-123 完成 12 项 `migrate_document` 候选的逐项复审，最终为 8 项 P4、3 项 P6 和 1 项 P7。3 项 P4/P6 职责分歧均由第三方逐项只读裁决：checkpoint blocker、builder readiness、量化核对和已测 rows/telemetry/full-engine gate 的制品、scope redline 与验证记录归 P4；checkpoint recovery、多卡训练/评测、scale-aware route 修复与 completion queue/runner 编排归 P6。独立安全复核确认 1 项真实凭据和私有运行信息暴露，保持 P7：必须确认旧凭据已轮换失效、移除明文与私有路径、采用环境变量示例并完成泄露回归后才可重审；本记录不声称这些条件已完成。当前真实唯一覆盖为 **1,244/1,551**，剩余 307 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-123 记录](P3_BATCH_123_MEASUREMENT_AND_CREDENTIAL_REAUDIT.md)。
...
**最新更正（覆盖上段的当前计数）**：P3-126 完成 12 项 `migrate_document` 候选的逐项复审，最终为 4 项 P4 和 8 项 P6，双独立审查逐项一致。QxS 自动后端关键点、Gold96/Gold176 与 Feedback16 的已测结果、行级证据、数据充分性和声明边界归 P4；后端对照、QxS 结构/能力扫描、Route-B INT8 修复、Gold Cold-start 批量执行及阶段 5 搜索/预算协议的构建、测量、修复和调度编排归 P6。已脱敏远程占位符、环境变量与泛化访问描述不构成 P7，但未来公开改写须去除远程访问和内部运行叙述。当前真实唯一覆盖为 **1,280/1,551**，剩余 271 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-126 记录](P3_BATCH_126_QXS_AND_SEARCH_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-127 完成 12 项 `migrate_document` 候选的逐项复审，最终为 5 项 P4 和 7 项 P6。7 项职责或安全边界分歧均由第三方逐项只读裁决：Stage5 的 AP/cold-start、独立复测与正式结果来源，六臂与 F-Cooper 的正式结果/声明边界，以及边缘测量映射的结果证据归 P4；Stage5 反馈纠正、CPU replay/诊断、后端补齐、边缘测量 runner 修复/表格回填和 Stage7 消融/恢复/三线实验编排归 P6。变量、占位符和省略根的远程/权限/路径叙述不构成 P7，但未来公开改写仍须泛化运行细节。当前真实唯一覆盖为 **1,292/1,551**，剩余 259 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-127 记录](P3_BATCH_127_STAGE5_STAGE7_AND_EDGE_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-128 完成 12 项 `migrate_document` 候选的逐项复审，最终为 5 项 P4、3 项 P6 和 4 项 P7。8 项职责或安全边界分歧均由第三方逐项只读裁决：Pareto 两版、SMBO 收敛、Phase-1 正式记录和 AP/量化闭环证据归 P4；分阶段实验路线、精度路由修复和批量 AP 补齐/导入编排归 P6。4 项历史凭据访问工作流或私有测量来源事实保持 P7：须确认历史凭据已轮换、撤销或失效，改写为非可执行且无身份的说明，移除私有运行信息并完成当前树、相关历史和导出物泄露回归后重审；本记录不声称已解除。当前真实唯一覆盖为 **1,304/1,551**，剩余 247 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-128 记录](P3_BATCH_128_PARETO_AND_SECURITY_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-129 完成 12 项 `migrate_document` 候选的逐项复审，最终为 3 项 P4、3 项 P6 和 6 项 P7。3 项职责或安全边界分歧均由第三方逐项只读裁决：异常测量策略、冻结训练数据记录和 cost-model 交叉验证证据归 P4；搜索/微调闭环、多工作器 LUT 生产和 replay/smoke/校准/数据生成编排归 P6。6 项私有运行信息、凭据材料或远程身份/访问工作流保持 P7：须完成非可执行公开改写、私有信息清除、泄露回归及新的独立审查，涉及凭据材料者还须失效确认；本记录不声称已解除。当前真实唯一覆盖为 **1,316/1,551**，剩余 235 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-129 记录](P3_BATCH_129_DATA_AND_RUNTIME_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-130 完成 12 项 `migrate_document` 候选的逐项复审，最终为 2 项 P4、3 项 P6 和 7 项 P7。2 项职责分歧均由第三方逐项只读裁决：最终实验/论文表格 provenance 与静态标准卷积普查结果归 P4；多代理 LUT 队列、标准卷积 probe/预测器升级和 Stage1 分区/集成合同归 P6。7 项远程身份/访问材料、私有运行或测量来源信息，或外部仿真项目来源、再分发和结果权限未确认的文档保持 P7：须完成安全改写、来源/许可确认、泄露回归和独立审查后重审；本记录不声称已解除。当前真实唯一覆盖为 **1,328/1,551**，剩余 223 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-130 记录](P3_BATCH_130_STAGE1_AND_SIMULATION_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-131 完成 12 项 `migrate_document` 候选的逐项复审，12 项均为 P7。风险包括远程身份/访问流程、私有运行或测量来源、凭据材料、特权操作，以及外部模型/数据/仿真/结果来源与再分发许可边界。须完成凭据失效确认（适用时）、非可执行脱敏改写、私有信息清除、来源/许可证确认、泄露回归和新的独立复审；本记录不声称任何阻塞已解除。当前真实唯一覆盖为 **1,340/1,551**，剩余 211 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-131 记录](P3_BATCH_131_ACCESS_AND_HANDOFF_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-132 完成 12 项 `migrate_document` 候选的逐项复审，最终为 2 项 P4、0 项 P6 和 10 项 P7。q6、q9 的初始 P7 分歧经第三方裁决改为 P4；其余 10 项保持 P7，须完成安全改写、来源/许可证确认、泄露回归和独立复审，涉及凭据者还须失效确认，本记录不声称已解除。当前真实唯一覆盖为 **1,352/1,551**，剩余 199 项文档候选；P3 仍在进行中，P4 尚未开始。详见 [P3-132 记录](P3_BATCH_132_CODRIVING_AND_PROVENANCE_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-133 完成 12 项候选，最终 P4=4、P6=7、P7=1。q6 含实际凭据材料，须轮换/失效确认、移除并完成当前/历史/导出物扫描后重审；其余文档按第三方裁决归 P4/P6。当前真实唯一覆盖 **1,364/1,551**，剩余 187 项；P3 仍在进行中，P4 尚未开始。详见 [P3-133 记录](P3_BATCH_133_TVM_ATTENTION_REAUDIT.md)。
...
**最新更正（覆盖此前当前计数）**：P3-140 完成 12 项 `migrate_document` 的双独立逐项审查与第三方裁决，最终 P4=8、P6=2、P7=1，另有 1 项 `duplicate_or_superseded`。q4 是字节级相同的两个 TRB README；q8/q12 是执行管线；q11 因第三方 Bench2Drive 来源/许可证证据缺失保持 P7；q6 虽标记私有标识风险但归 P4。受限账本与 acceptance manifest 已重算，真实唯一覆盖 **1,448/1,551**，剩余 **103** 项；P3 仍进行中，P4 尚未开始，未推送、未发布。

**最新更正（覆盖上段的当前计数）**：P3-141 完成 12 项 `migrate_document` 的双独立逐项审查与第三方裁决，最终 P4=6、P6=5、P7=1，无重复项。仿真/测量来源和论文笔记归 P4；H800 IPC 运行设计、两份不同的冻结路线 fixture 以及剪枝/量化/TRT/评测/搜索执行计划归 P6；含私有论文 PDF 目录与未确认外部许可的 joint-search 文档保持 P7。受限账本与 acceptance manifest 已重算，真实唯一覆盖 **1,460/1,551**，剩余 **91** 项；P3 仍进行中，P4 尚未开始，未推送、未发布。详见 [P3-141 记录](P3_BATCH_141_SIMULATION_PROVENANCE_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-142 完成 12 项 `migrate_document` 的双独立逐项审查与第三方裁决，最终 P4=4、P6=7、P7=1，无重复项。量化/剪枝历史结果和反思归 P4；A1/A2/A3/B1/B2 实现、验证、详细实施计划和完整实验流程归 P6；剪枝完成报告因私有工作树路径及 checkpoint/数据/GPU 资产边界保持 P7。受限账本与 acceptance manifest 已重算，真实唯一覆盖 **1,472/1,551**，剩余 **79** 项；P3 仍进行中，P4 尚未开始，未推送、未发布。详见 [P3-142 记录](P3_BATCH_142_CONFIGURABLE_QUANT_PRUNE_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-143 完成 12 项 `migrate_document` 的双独立逐项审查与第三方裁决，最终 P4=2、P6=8、P7=2，无重复项。剪枝最终进展和状态汇总归 P4；Phase A–D、B1/C1 联合压缩及两个实施计划归 P6；两个同哈希 Phase-D 文档均因绝对本机路径和外部资产命令保持 P7，不能仅凭内部同哈希记为重复。受限账本与 acceptance manifest 已重算，真实唯一覆盖 **1,484/1,551**，剩余 **67** 项；P3 仍进行中，P4 尚未开始，未推送、未发布。详见 [P3-143 记录](P3_BATCH_143_CONFIGURABLE_PRUNING_REAUDIT.md)。

**最新更正（覆盖上段的当前计数）**：P3-144 完成 12 项 `migrate_document` 的双独立逐项审查与第三方裁决，最终 P4=9、P6=1、P7=2，无重复项。剪枝实验/结果、搜索空间和硬件优化进展归 P4；硬件优化实施计划归 P6；两个 RALPH 提示因绝对本机路径和可执行代理命令保持 P7。受限账本与 acceptance manifest 已重算，真实唯一覆盖 **1,496/1,551**，剩余 **55** 项；P3 仍进行中，P4 尚未开始，未推送、未发布。详见 [P3-144 记录](P3_BATCH_144_HARDWARE_OPTIMIZATION_REAUDIT.md)。
...
P3-134 已记录为上一批：P4=9、P6=3、P7=0，覆盖 1,376/1,551。
