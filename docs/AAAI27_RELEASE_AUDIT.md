...
**状态日期：2026-08-17；此处记录 P6.2 动态框架候选空间离线接入完成后的公开交接状态。** 当前工作分支为 `p6-h800-search-execution`。P1 最终候选 `c65b5fc7eb9d20e2d34928f7c948aea90f822754` 及其 tree `e9adfe9888a21ae0b5f819858e1cea001acbcecb` 仍只作为 P1 冻结证据；后续 P2--P6.2 离线状态由本台账追加记录约束。实时远端差异、工作树状态和新候选提交必须以 `git status --short --branch` 与当次验证输出为准，不能复用旧冻结数字。
...
| 完整项目源代码 | P3、P4、P5 已完成（本地）；P6.1 静态 343×2 历史路径已完成（本地）；P6.2 动态框架候选空间离线接入与验证已完成（本地） | 当前树含公开的接口、验证、选择、聚合、外部输入登记、环境契约、P6.1 本地执行边界文档和 P6.2 动态离线契约；P6.1 只记录已完成的本地历史路径，P6.2 从 Stage2 动态派生候选池而不预设为 343 或 686。 | 真实框架来源闭环仍待本地执行；P7 安全/合规/供应链审查和 P8 发布候选验证尚未开始。 |
...
| 公开发布 | 未开始 | P1 本地发布候选、P3 本地关闭、P4/P5 本地收口、P6.1 静态历史收口及 P6.2 动态离线接入验证均有台账记录；尚未执行合并、tag、Release 或仓库可见性变更。 | 需要完成真实框架来源闭环、P7 和 P8，并在最后取得推送/发布的明确授权。 |
...
| P6 | P6.1 静态 343×2 历史路径已完成（本地）；P6.2 动态框架候选空间离线接入与验证已完成（本地） | P6.1 的静态 343×2 是已完成的本地历史路径。P6.2 从 Stage2 动态派生候选池，不预设为 343 或 686；单个候选不是 mixed-precision。离线完成只确认动态候选空间的实现和验证，真实框架来源闭环仍待本地执行。Orin 与 TensorRT 仍为后续独立线。 | P6.1 历史收口和 P6.2 离线完成都不生成公开 P6 结果摘要，也不等同于真实框架来源闭环、Stage6 或 Stage7 论文证据完成。Git 忽略的本地执行材料不进入公开面或匿名归档；不公开候选标识、执行位置、命令、结果、检查点或日志。 |
...
5. P6.1 静态 343×2 是已完成的本地历史路径；该收口不公开逐轮结果，也不将其提升为 Stage6/Stage7 论文证据。P6.2 动态框架候选空间从 Stage2 派生，离线接入与验证已完成，但真实框架来源闭环仍待本地执行。
