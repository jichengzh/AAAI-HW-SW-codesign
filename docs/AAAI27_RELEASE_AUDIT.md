...
| 完整项目源代码 | P3、P4、P5 已完成（本地）；P6.1 已完成（本地）；P6.2 待开始 | 当前树含公开的接口、验证、选择、聚合、外部输入登记、环境契约和 P6.1 本地执行边界文档；P6.1 只证明 Pyramid/H800/TVM 闭环可由授权本地资产驱动并完成四轮反馈。 | P6.2 framework search-space integration 尚未实施；P7 安全/合规/供应链审查和 P8 发布候选验证尚未开始。 |
...
| P6 | P6.1 已完成（本地）；P6.2 待开始 | P6.1 已完成 Pyramid/H800/TVM 的 CoptV2X 完整搜索闭环：冻结 176 条 Gold cold-start 证据，以完整 capability profile 上下文学到跨配置趋势；其中 174 条成功实测记录重训 cost model、两条真实可行性失败仅保留为证据且不伪造指标或重测。候选预测、选点和真实测量严格固定为 H800/TVM；基于 343×2 候选注册表完成 4 轮、每轮 4 个候选的本地执行与反馈重训，控制器终态及每轮反馈均已核验。P6.2 的 framework search-space integration、Orin 与 TensorRT 均为后续独立线。 | P6.1 的完成结论只记录脱敏结构性执行事实，不生成公开 P6 结果摘要，也不等同于 Stage6 或 Stage7 论文证据完成。本地忽略配置保存资产路径、命令和原始输出；公开面只保留固定契约、脱敏资产标签、版本/许可状态和离线验证器。不下载资产，不公开路径、命令、主机信息、候选 ID、原始日志、checkpoint、编译产物或逐轮原始结果；共享资产/命令/结果结构失败必须整批隔离，候选真实可行性失败才消耗预算。 |
