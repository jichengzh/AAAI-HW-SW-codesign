...
**P3-138 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方裁决分歧；12 项全部为 P4 `paper_source_boundary`。评测/仿真定义不含运行时 caller、runner API、作业队列或执行 I/O；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,424/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。
