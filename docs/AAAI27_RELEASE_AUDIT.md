...
| P6 | 进行中（本地） | 第一阶段建立并验证 H800 自动搜索闭环：在维护者已本地准备的资产上完成候选选择、训练/评测、反馈与结果汇总；Orin 是后续独立执行线。 | 本地忽略配置保存路径、命令和原始输出；公开面只保留脱敏资产标签、版本/许可状态、代码版本、seed、配置标签、状态和聚合指标。不下载资产，不公开路径、命令、主机信息、原始日志、checkpoint 或逐轮原始结果；缺失资产或执行失败必须失败关闭。 |
