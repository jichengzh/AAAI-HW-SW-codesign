...
| 公开发布 | 未开始 | P1 本地发布候选、P3 本地关闭、P4/P5 本地收口、P6.1 静态历史收口、P6.2 动态离线接入验证、P6.3 历史执行适配器离线验证，以及本轮 RTX4090 迁移公开门禁均有台账记录；尚未执行合并、tag、Release 或仓库可见性变更。 | 需要完成真实 RTX4090 四卡四轮验证、P7 和 P8，并在最后取得推送/发布的明确授权。 |
...
| P6 | P6.1 静态 343×2 历史路径已完成（本地）；P6.2 动态框架候选空间离线接入与验证已完成（本地）；P6.3 历史执行适配器已实现并通过离线验证；历史执行根归一桥接实现完成；RTX4090 硬件 profile 参数化和私有链路 smoke/probe 正在收口 | P6.1 的静态 343×2 是已完成的本地历史路径。P6.2 从 Stage2 动态派生候选池，不预设为 343 或 686；单个候选不是 mixed-precision。P6.3 历史执行适配器已实现并通过离线验证。归一桥接的公开离线门禁覆盖 normalizer→provision→真实 Stage1 manifest→动态 Stage2 plan→registry-v2。RTX4090 迁移使用 `configs/execution/p6_rtx4090_search.example.yaml` 和历史名称 controller 入口，不新增 RTX-specific verifier。 | 真实 RTX4090 四卡四轮执行和 verifier 结果仍待本地运行完成；RTX4090 机制验证不得重标记为 H800 论文数值。Git 忽略的本地执行材料不进入公开面或匿名归档；不公开候选标识、执行位置、命令、结果、检查点或日志。 |
...
#### P3-76：TensorRT/TVM recovery 边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 3 项 P6（TensorRT engine repair/build/inspect/CUDA re-benchmark 与 TVM recovery measurement）和 9 项 P7（私有模型、workdir、结果、CUDA/PTX dump 或环境文件根目录绑定）。q4、q5、q12 经第三次裁决：消费 engine/cache 或 workdir 不改变其实际 build/inspect/compile/execute/measure 的主导执行职责。该批新增 12 个不重复身份，使真实唯一覆盖到 **686/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-76 记录](release-manifests/P3_BATCH_76_TRT_TVM_RECOVERY_REAUDIT.md)。

#### P3-77：Pareto 制品与 TensorRT 边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 9 项 P4（latency/AP/预测器、ONNX 或校准制品及其派生产物）、1 项 P6（TensorRT/CUDA engine build/inspect）、1 项 P7（私有运行工作目录绑定）和 1 项公开实现已替代。q3、q12 经第三次裁决：公开 latency-LUT API、调用链和 fixture 测试已完整承担查询/插值职责；构建和检查 TensorRT engine 的主导边界仍为执行。该批新增 12 个不重复身份，使真实唯一覆盖到 **698/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-77 记录](release-manifests/P3_BATCH_77_PARETO_ARTIFACT_AND_TRT_REAUDIT.md)。

#### P3-78：TensorRT/CUDA 测量边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 2 项 P4（外部评测日志与多模型 Pareto 结果制品）、9 项 P6（TensorRT engine build/load/execute/inspect 及 CUDA、NVML、CUDA Graph、设备采样或能耗/延迟测量）和 1 项 P7（第三方 NMS monkey-patch 的来源、许可证和 NOTICE 链路）。q2--q10 经第三次裁决：即使消费外部 ONNX、engine 或校准制品，实际 build/execute/measure/inspect 脚本的主导边界仍是 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **710/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-78 记录](release-manifests/P3_BATCH_78_TRT_CUDA_MEASUREMENT_REAUDIT.md)。

#### P3-79：耦合与 TVM 执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项 P4（外部 screening 制品解析）、6 项 P6（TensorRT/TVM build/load/execute/inspect、CUDA、MetaSchedule 或测量）和 5 项 P7（私有模型、数据、工作目录或第三方来源/许可边界）。q1、q2、q3、q7、q8、q10、q11 经第三次裁决：私有输入/许可边界优先 P7；合成算子只要实际执行 CUDA/TVM 调优、编译或运行，主导边界仍为 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **722/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-79 记录](release-manifests/P3_BATCH_79_COUPLING_AND_TVM_REAUDIT.md)。

#### P3-80：TVM kernel 执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 10 项 P6（TVM/CUDA compile、MetaSchedule tune、VM run、计时、TIR 导出或结果记录）和 2 项 P7（私有调优工作目录或外部 ONNX 输入绑定）。q1--q6、q8、q9、q11、q12 经第三次裁决：环境依赖本身不足以归 P5，脚本若自身承担 compile/tune/run/measure 即归 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **734/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-80 记录](release-manifests/P3_BATCH_80_TVM_KERNEL_EXECUTION_REAUDIT.md)。

#### P3-81：预测器与 Stage1 公开链复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 2 项公开迁入、1 项安全公开改写、5 项 P6（ONNX/TVM build/run 或 LightGBM 训练、评估、模型/指标导出）和 4 项 P7（私有工作目录、第三方训练/算子来源或错误输出回显风险）。q2、q9--q12 经第三次裁决：读取数据后自行训练、评估和导出模型的脚本属于 P6，而非仅外部制品整合。该批新增 12 个不重复身份，使真实唯一覆盖到 **746/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-81 记录](release-manifests/P3_BATCH_81_PREDICTOR_AND_STAGE1_REAUDIT.md)。

#### P3-82：Phase4 制品与训练边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 7 项 P4（外部数据、日志、硬件结果、配置和指标制品的校验、解析或聚合）、4 项 P6（LightGBM 训练、交叉验证/评估与模型/指标导出）和 1 项 P7（第三方 VFE 微基准来源与改写许可风险）。q1--q4 经第三次裁决：外部训练数据不改变脚本自身训练、评估和模型导出的 P6 主导职责。该批新增 12 个不重复身份，使真实唯一覆盖到 **758/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-82 记录](release-manifests/P3_BATCH_82_PHASE4_AND_TRAINING_REAUDIT.md)。

#### P3-83：Phase4 训练、Stage1/Stage2 公开链复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 3 项安全公开改写、4 项 P4（历史测量、AP 来源或制品 registry 的脱敏公开契约）和 5 项 P6（LightGBM 训练、交叉验证/评估与模型/指标导出）。q2--q6 经第三次裁决：外部 CSV 只是输入依赖，不改变训练/验证/模型导出脚本的 P6 主导职责。该批新增 12 个不重复身份，使真实唯一覆盖到 **770/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-83 记录](release-manifests/P3_BATCH_83_PHASE4_STAGE1_STAGE2_REAUDIT.md)。

#### P3-84：CoDriving TVM 执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 3 项 P4（历史架构审计、诊断报告或 INT8 来源证明等外部证据制品）和 9 项 P6（ONNXRuntime/TVM import、build、compile、tune、VM run、数值对齐、计时或能耗测量）。q3、q4、q5、q7--q12 经第三次裁决：外部 ONNX、校准或参考制品属于输入约束，不改变脚本自身执行职责；OpenCOOD 标记仅指形状/配置来源，静态未见明确代码复制或移植证据。该批新增 12 个不重复身份，使真实唯一覆盖到 **782/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-84 记录](release-manifests/P3_BATCH_84_CODRIVING_TVM_REAUDIT.md)。

#### P3-85：Stage2 覆盖、队列与 LUT 边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项非必要私有 CLI 排除、5 项 P4（外部测量、制品 registry、探测运行或候选计划）和 6 项 P6（TVM/CUDA worker、外部 telemetry/latency 命令或硬件测量执行调度队列）。q1、q4--q7、q10、q11 经第三次裁决：公开 API/测试与既有范围策略完整覆盖私有 outlier 文件 I/O CLI；只生成候选计划而没有命令链的脚本归 P4，具有硬件测量调度语义的队列归 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **794/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-85 记录](release-manifests/P3_BATCH_85_STAGE2_COVERAGE_AND_LUT_REAUDIT.md)。

#### P3-86：H800 worker 与 LUT 边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项安全公开改写、4 项 P4（外部 profile、测量回放或 LUT 导入契约）、3 项 P6（可执行 H800 测量调度或 TVM/CUDA 编译/worker 执行）和 4 项 P7（私有运行环境、个人路径、任意命令入口或未经脱敏的命令/主机/traceback 日志）。q1、q8、q9、q11 经第三次裁决：可执行硬件命令链属于 P6；带私有 runtime 与任意命令/日志风险的 worker 优先 P7。该批新增 12 个不重复身份，使真实唯一覆盖到 **806/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-86 记录](release-manifests/P3_BATCH_86_H800_WORKERS_AND_LUT_REAUDIT.md)。

#### P3-87：制品计划与结构探测边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 6 项 P4（外部制品、证据、probe、readiness 与历史回放报告）、3 项 P6（实时 GPU launch gate、可执行 LUT 命令链或 TVM/TensorRT build）和 3 项 P7（私有制品或 TVM/CUDA 环境根及未脱敏主机/日志输出）。q2、q6、q12 经第三次裁决：实时 GPU 调度、可执行命令链和实际编译均属 P6，而非纯环境描述。该批新增 12 个不重复身份，使真实唯一覆盖到 **818/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-87 记录](release-manifests/P3_BATCH_87_ARTIFACT_AND_PROBE_REAUDIT.md)。

#### P3-88：SMBO 与 CoDriving 执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项安全公开改写、4 项 P4（外部 telemetry、LUT、队列状态与 registry）、4 项 P6（SMBO/成本模型训练或 TensorRT build/execute）和 3 项 P7（远端凭据同步或私有 CoDriving 数据、模型、路径及许可）。q2、q3、q5、q6、q7、q11 经第三次裁决：训练/模型导出及实际 TRT build 属 P6，纯状态汇总归 P4，凭据与远端同步风险优先 P7。该批新增 12 个不重复身份，使真实唯一覆盖到 **830/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-88 记录](release-manifests/P3_BATCH_88_SMBO_AND_CODRIVING_REAUDIT.md)。

#### P3-89：制品与权限边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 10 项 P4（registry、measurement rows、汇总、模型制品、Gold 数据集、报告和审计记录等外部制品契约）和 2 项 P7（第三方协同驾驶数据、检查点、运行时或硬件身份输出的来源、许可证、权限与脱敏阻塞）。双独立只读审查对全部逐项结论一致，无需第三次裁决。该批新增 12 个不重复身份，使真实唯一覆盖到 **842/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-89 记录](release-manifests/P3_BATCH_89_ARTIFACT_AND_PERMISSION_REAUDIT.md)。

#### P3-90：Gold 计划与充分性边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 4 项 P4（Gold 数据集、重复测量与完整性审计等外部制品契约）和 8 项 P6（可执行性能/AP 计划或实际离线充分性模型训练/评估）。q2、q3、q5、q7、q11、q12 经第三次只读裁决：可执行任务计划和模型 `.fit()`/`.predict()` 训练评估属 P6，而非纯环境或制品汇总。该批新增 12 个不重复身份，使真实唯一覆盖到 **854/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-90 记录](release-manifests/P3_BATCH_90_GOLD_PLAN_AND_SUFFICIENCY_REAUDIT.md)。

#### P3-91：Gold 与 Closure 边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项安全公开改写、6 项 P4（校准、Gold 数据集、pilot、修复证据与 readiness 等外部制品契约）、4 项 P6（可执行性能/AP 计划、离线充分性模型训练或量化推理）和 1 项 P7（私有运行环境默认值阻塞）。q2、q4、q8、q10、q11、q12 经第三次只读裁决；公开 closure rewrite 由公开纯 API 与脱敏测试完整承接，运行环境风险项优先归 P7。该批新增 12 个不重复身份，使真实唯一覆盖到 **866/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-91 记录](release-manifests/P3_BATCH_91_GOLD_AND_CLOSURE_REAUDIT.md)。

#### P3-92：Stage4/Stage5 执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 1 项 P4（反馈制品训练视图合并）和 11 项 P6（选择、成本模型或反馈更新评估，性能/AP 作业计划，或在线搜索与测量请求推进）。q1、q2、q4、q5、q7、q8 经第三次只读裁决：公开 API 的存在不足以替代候选本身的实际训练、评估或执行调度职责，故归 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **878/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-92 记录](release-manifests/P3_BATCH_92_STAGE4_STAGE5_EXECUTION_REAUDIT.md)。

#### P3-93：Stage5 证据与回放边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 7 项 P4（实际反馈、独立验证、审计、闭合与图特征等外部制品契约）和 5 项 P6（执行状态修复、性能作业计划、在线搜索重放或测量请求初始化）。q1、q3、q4、q11 经第三次只读裁决：即使读取既有制品，含可执行作业或实际拟合/预测/选择的职责仍归 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **890/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-93 记录](release-manifests/P3_BATCH_93_STAGE5_EVIDENCE_AND_REPLAY_REAUDIT.md)。

#### P3-94：Stage5/Stage6 证据与执行边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。最终为 6 项 P4（actual-feedback、实验、AP 和论文表格等外部证据契约）和 6 项 P6（实际搜索初始化、独立验证性能作业或 hidden-label replay）。q4、q6 经第三次只读裁决：生成可执行 jobs 或实际拟合/预测/选择的职责归 P6。该批新增 12 个不重复身份，使真实唯一覆盖到 **902/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-94 记录](release-manifests/P3_BATCH_94_STAGE5_STAGE6_REAUDIT.md)。

#### P3-95：Stage6 证据边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。独立复审首次候选身份集合与固定队列不一致，已整体作废并按固定队列重新逐项审阅；最终为 10 项 P4（Stage6 证据、验证、烟雾门控和 formal 状态等外部制品契约）和 2 项 P6（TVM/CUDA 编译导出或 formal 测量请求生成）。q1、q4、q8、q11 经第三次只读裁决。该批新增 12 个不重复身份，使真实唯一覆盖到 **914/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-95 记录](release-manifests/P3_BATCH_95_STAGE6_EVIDENCE_REAUDIT.md)。

#### P3-96：Stage6 执行与证据边界复审（已接受，本地）

12 项完成 P2 metadata/bytes、当前内容绑定、主审、独立语义复审、项目级许可证继承和安全输出审查。主审与独立复审均先核验同一固定队列 SHA；最终为 5 项 P4（launch gate、repair promotion、baseline reseal 或审计视图等外部制品契约）和 7 项 P6（独立验证 rerun jobs、正式执行计划或 TVM/TensorRT GPU 构建探测）。q1、q2、q6、q7、q11、q12 经第三次只读裁决。该批新增 12 个不重复身份，使真实唯一覆盖到 **926/1,551**。没有执行私有模型、数据、设备、子进程或网络任务；P4 尚未开始。详见 [P3-96 记录](release-manifests/P3_BATCH_96_STAGE6_EXECUTION_AND_EVIDENCE_REAUDIT.md)。
...
5. P6.1 静态 343×2 是已完成的本地历史路径；该收口不公开逐轮结果，也不将其提升为 Stage6/Stage7 论文证据。P6.2 动态框架候选空间从 Stage2 派生，离线接入与验证已完成。P6.3 历史执行适配器已实现并通过离线验证；真实 H800 框架来源四轮执行仍待本地运行。
...
| 2026-08-05 | P3-76 TensorRT/TVM recovery 边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 3 项 P6、9 项 P7；q4、q5、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 686/1,551；未推送、未发布。 |
| 2026-08-05 | P3-77 Pareto 制品与 TensorRT 边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 9 项 P4、1 项 P6、1 项 P7、1 项公开实现已替代；q3、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 698/1,551；未推送、未发布。 |
| 2026-08-05 | P3-78 TensorRT/CUDA 测量边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 2 项 P4、9 项 P6、1 项 P7；q2--q10 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 710/1,551；未推送、未发布。 |
| 2026-08-05 | P3-79 耦合与 TVM 执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 1 项 P4、6 项 P6、5 项 P7；q1、q2、q3、q7、q8、q10、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 722/1,551；未推送、未发布。 |
| 2026-08-05 | P3-80 TVM kernel 执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 10 项 P6、2 项 P7；q1--q6、q8、q9、q11、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 734/1,551；未推送、未发布。 |
| 2026-08-05 | P3-81 预测器与 Stage1 公开链复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 2 项公开迁入、1 项安全公开改写、5 项 P6、4 项 P7；q2、q9--q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 746/1,551；未推送、未发布。 |
| 2026-08-05 | P3-82 Phase4 制品与训练边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 7 项 P4、4 项 P6、1 项 P7；q1--q4 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 758/1,551；未推送、未发布。 |
| 2026-08-05 | P3-83 Phase4 训练、Stage1/Stage2 公开链复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 3 项安全公开改写、4 项 P4、5 项 P6；q2--q6 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 770/1,551；未推送、未发布。 |
| 2026-08-05 | P3-84 CoDriving TVM 执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 3 项 P4、9 项 P6；q3、q4、q5、q7--q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 782/1,551；未推送、未发布。 |
| 2026-08-05 | P3-85 Stage2 覆盖、队列与 LUT 边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 1 项非必要 CLI 排除、5 项 P4、6 项 P6；q1、q4--q7、q10、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 794/1,551；未推送、未发布。 |
| 2026-08-05 | P3-86 H800 worker 与 LUT 边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 1 项安全公开改写、4 项 P4、3 项 P6、4 项 P7；q1、q8、q9、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 806/1,551；未推送、未发布。 |
| 2026-08-05 | P3-87 制品计划与结构探测边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开反证、项目级许可与安全输出审查。最终 6 项 P4、3 项 P6、3 项 P7；q2、q6、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 818/1,551；未推送、未发布。 |
| 2026-08-05 | P3-88 SMBO 与 CoDriving 执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 1 项安全公开改写、4 项 P4、4 项 P6、3 项 P7；q2、q3、q5、q6、q7、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 830/1,551；未推送、未发布。 |
| 2026-08-05 | P3-89 制品与权限边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 10 项 P4、2 项 P7；双独立只读审查逐项一致。HMAC ledger 已重新计算，真实唯一覆盖为 842/1,551；未推送、未发布。 |
| 2026-08-05 | P3-90 Gold 计划与充分性边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 4 项 P4、8 项 P6；q2、q3、q5、q7、q11、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 854/1,551；未推送、未发布。 |
| 2026-08-05 | P3-91 Gold 与 Closure 边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 1 项安全公开改写、6 项 P4、4 项 P6、1 项 P7；q2、q4、q8、q10、q11、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 866/1,551；未推送、未发布。 |
| 2026-08-05 | P3-92 Stage4/Stage5 执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 1 项 P4、11 项 P6；q1、q2、q4、q5、q7、q8 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 878/1,551；未推送、未发布。 |
| 2026-08-05 | P3-93 Stage5 证据与回放边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 7 项 P4、5 项 P6；q1、q3、q4、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 890/1,551；未推送、未发布。 |
| 2026-08-05 | P3-94 Stage5/Stage6 证据与执行边界复审 | 已接受（本地） | 12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 6 项 P4、6 项 P6；q4、q6 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 902/1,551；未推送、未发布。 |
| 2026-08-05 | P3-95 Stage6 证据边界复审 | 已接受（本地） | 首次独立复审候选身份不一致，已整体作废并重审；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 10 项 P4、2 项 P6；q1、q4、q8、q11 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 914/1,551；未推送、未发布。 |
| 2026-08-05 | P3-96 Stage6 执行与证据边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列 SHA；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 5 项 P4、7 项 P6；q1、q2、q6、q7、q11、q12 经第三次裁决。HMAC ledger 已重新计算，真实唯一覆盖为 926/1,551；未推送、未发布。 |
| 2026-08-05 | P3-97 Stage7 执行与证据边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 4 项 P4、8 项 P6；q1、q3、q7、q9 经第三次裁决，一项非法“非必要排除→P7”组合已拒绝并按执行边界改为 P6。HMAC ledger 已重新计算，真实唯一覆盖为 938/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-98 Stage7 运行时与契约边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 2 项 P4、2 项 P5、7 项 P6、1 项非必要排除；q4--q6、q8、q11、q12 经第三次裁决，两个没有真实公开实现与测试的候选未被提前记为已迁入/改写，而转为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 950/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-99 Phase2 测试与调试边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 6 项 P4、2 项 P6、4 项非必要排除；q1--q8 经第三次裁决，五项非法“非必要排除→P7”组合已拒绝，且未把未来可迁入的私有测试提前记为已改写。HMAC ledger 已重新计算，真实唯一覆盖为 962/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-100 分析工具与调试边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 5 项 P4、1 项 P6、6 项非必要排除；两个上游许可证/职责分歧经第三次裁决，Apache-2.0 来源依据充分，分别归 P4/P6 而非 P7，且没有候选被提前迁入。HMAC ledger 已重新计算，真实唯一覆盖为 974/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-101 TensorRT、TVM 与校准边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 2 项 P4、10 项 P6；q1--q9、q11 经第三次裁决，实际 build/run/tune/measure/calibrate 工具归 P6，本地路径/输出风险不构成 P7。HMAC ledger 已重新计算，真实唯一覆盖为 986/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-102 量化、评测与推理边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 6 项 P4、6 项 P6；q1、q2、q4--q6、q8、q11 经第三次裁决，缺少公开实现/测试链的计划与私有测试归 P4，实际检查/评测/导出/测时归 P6。HMAC ledger 已重新计算，真实唯一覆盖为 998/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-103 Orin 对齐与剪枝边界复审 | 已接受（本地） | 主审与独立复审均先核验同一固定队列身份；12 项完成 P2/当前内容、职责、调用、公开等价物、项目级许可与安全输出审查。最终 7 项 P4、5 项 P6；q2、q4--q6、q9 经第三次裁决，纯报告解析与未验证等价性制品归 P4，实际 Orin/TensorRT/模型测量归 P6。HMAC ledger 已重新计算，真实唯一覆盖为 1,010/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-104 最后代码候选边界复审 | 已接受（本地） | 最后 8 项 `migrate_code` 均完成固定队列身份核验、双独立逐项审查；最终 1 项 P4、7 项 P6。TRT head smoke 经第三次裁决，实际 CUDA dummy forward 归 P6。代码候选已审阅穷尽，后续转入配置候选。HMAC ledger 已重新计算，真实唯一覆盖为 1,018/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-105 配置职责边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；主审与独立复审的全部逐项分歧均经第三次裁决。最终 1 项 P5、10 项 P6、1 项非必要排除：硬件能力/工具链描述归 P5，真实剪枝、量化或评测流水线输入归 P6，无支持链调用的内部审查收据可排除。HMAC ledger 已重新计算，真实唯一覆盖为 1,030/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-106 主动采样配置边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 12 项 P6：实际结构剪枝参数与量化路径无剪枝控制面均直接进入真实剪枝、量化、评测或执行路由，静态 JSON 形式或暂无公开等价物均不足以改记为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,042/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-107 剪枝配置职责边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 12 项 P6：配置直接进入结构剪枝、微调、量化、评测或执行路由，其实验契约属性、静态 JSON 形式或暂无公开等价物均不足以自动改记为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,054/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-108 剪枝预设职责边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 12 项 P6：解耦、强度扫描与扩展预设直接控制模型加载后的剪枝、微调、量化、评测、测速或执行路由；配置形式、实验矩阵用途或暂无公开等价物均不足以自动改记为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,066/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-109 Decoder 与量化配置职责边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 12 项 P6：decoder 层数直接变更模型结构，FP32/no-quant 配对文件也作为控制面决定实际执行路由；没有公开等价物不能自动改记为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,078/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-110 主动量化配置职责边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 12 项 P6：FP32/no-quant 配对文件决定实际执行分支，INT8/混合精度配置直接控制量化模型变换和评测路径；静态 JSON、实验矩阵用途或没有公开等价物均不足以自动改记为 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,090/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-111 量化配置与校准制品边界复审 | 已接受（本地） | 12 项 `migrate_config` 均完成固定队列身份核验、双独立逐项审查；全部主导职责分歧经第三次裁决。最终 11 项 P6、1 项 P4：量化配置直接决定 Q/DQ、模型变换、量化评测或执行路由；逐层校准尺度制品以来源、可公开性和可再生性为主导依赖。静态审阅未见 P7 阻塞，且缺少公开等价物不自动归 P4。HMAC ledger 已重新计算，真实唯一覆盖为 1,102/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-112 最后配置候选边界复审 | 已接受（本地） | 冻结基线最后 10 项 `migrate_config` 以自然数量入队，未补齐或重复；均完成固定队列身份核验、双独立逐项审查，全部分歧经第三次裁决。最终 4 项 P6、6 项 P4：量化执行配置归 P6，数据转换/标签更新制品归 P4；私有镜像但没有公开实现与测试/API职责证据不构成迁入或重复，未见 P7 阻塞。代码、fixture 与配置候选现已审阅穷尽，后续仅处理 439 项文档候选。HMAC ledger 已重新计算，真实唯一覆盖为 1,112/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-113 文档职责与公开边界复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；4 项职责分歧经第三次裁决。最终 2 项 P4、2 项 P5、6 项 P6、1 项 P7、1 项非必要排除：量化/导出/执行文档归 P6，资产/复现边界归 P4，环境/脱敏远程访问规范归 P5，瞬态机器状态交接因无支持链且其维护性职责已由既有公开发布状态与复现边界材料承担而排除；一份含未脱敏私有路径引用的文档保持 P7，公开记录不披露该路径。HMAC ledger 已重新计算，真实唯一覆盖为 1,124/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-114 文档执行、论文证据与安全边界复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；4 项职责分歧经第三次裁决。最终 5 项 P4、3 项 P6、4 项 P7：已结束量化实验的状态记录、量化结果和论文图表/表格规划以外部制品为主导，归 P4；Q/DQ 修复路线和会启动/编排 GPU lane 的 supervisor 计划归 P6；现有同主题公开模块、CLI 与测试未承担同职责的图表源数据/导出契约，不能提前记为重复或已迁入；4 份含未脱敏私有路径引用或外部执行引用的文档保持 P7，公开记录不披露这些引用。HMAC ledger 已重新计算，真实唯一覆盖为 1,136/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-115 研究设计、归档和运行入口复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；9 项职责分歧经第三次裁决。最终 7 项 P4、1 项 P6、1 项 P7、1 项已安全公开改写、1 项公开实现已替代、1 项非必要排除：论文/实验制品、研究资料索引和历史研究证据归 P4，闭环仿真编排归 P6；含私有远程访问参数、口令式认证流程和受控执行引用的运行入口保持 P7，公开记录不披露这些信息；只有有公开实现与测试/API同职责证据的 fixture 或桥接设计才关闭为已改写或已替代；旧版归档导航只有在无当前公开支持链、且维护性职责已由既有公开发布、复现材料和测试承担时才可排除，归档研究证据不因此排除。HMAC ledger 已重新计算，真实唯一覆盖为 1,148/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-116 硬件与方法设计复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；6 项职责分歧经第三次裁决。最终 8 项 P4、2 项 P5、1 项 P6、1 项 P7：硬件文献与各类实验/论文结果边界归 P4，平台能力、工具链和 TVM 硬件 gate 归 P5，实际模型/编译/评测流水线计划归 P6；泛化硬件路径或结果文件名不构成 P7，但确认含未脱敏私有路径引用的多模型探测计划保持 P7，公开记录不披露该引用。HMAC ledger 已重新计算，真实唯一覆盖为 1,160/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-117 自动调优证据与访问边界复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；9 项职责分歧经第三次裁决。最终 6 项 P4、3 项 P6、3 项 P7：受控测量、论文证据、制品登记和设计口径归 P4，实际 TVM 调优、LUT 生成与 worker/验证交接归 P6；真实私有路径、进程标识或远程连接/认证流程保持 P7，公开记录不披露这些信息。设计文本、泛化硬件词或进展文档不自动决定处置，均以实际主导职责和内容为准。HMAC ledger 已重新计算，真实唯一覆盖为 1,172/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
...
| 2026-08-05 | P3-122 Full-AP 与 Route 边界复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；3 项 P4/P6 职责分歧经第三次裁决。最终 3 项 P4、9 项 P6：已测 smoke、AP/route readiness 与 numeric/trace blocker 制品归 P4；full-AP 监控、bridge 修复与重启、gate 后导入、coverage 刷新、checkpoint recovery、route 建设及 AP/energy 补点归 P6。占位主机、环境变量和无变量名的脱敏片段不构成 P7，未发现真实凭据或连接秘密。HMAC ledger 已重新计算，真实唯一覆盖为 1,232/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-123 测量证据与凭据阻塞复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；3 项 P4/P6 职责分歧经第三次裁决。最终 8 项 P4、3 项 P6、1 项 P7：checkpoint/builder/量化核对及已测 rows/telemetry/full-engine gate 证据归 P4，checkpoint recovery、多卡训练/评测、scale-aware route 修复和 completion queue/runner 归 P6；1 项真实凭据和私有运行信息暴露保持 P7，须轮换失效确认、脱敏和泄露回归后重审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,244/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-124 加速证据与凭据工作流复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；8 项 P4/P6 职责分歧经第三次裁决。最终 8 项 P4、2 项 P6、2 项 P7：FP32 energy、INT8 不确定性、TensorCore/full-engine、rewritten engine、跨精度根因/scale-sweep 和 AP-shape/speed-gate 的已测证据归 P4，full-val 监控与 suite/门禁执行归 P6；2 项遗留凭据访问工作流和私有运行信息保持 P7，须失效确认、非可执行改写、脱敏与当前树/历史/导出物泄露回归后重审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,256/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-125 覆盖、后端与证据复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查，结论一致。最终 7 项 P4、5 项 P6：AP-shape matcher、三精度覆盖、历史缺口、后端比较更正、实验数据库/provenance、claim-evidence 与 INT8 cold-start 审计证据归 P4；Original60 补测、AP smoke worker、INT8 recovery、多框架比较与 TVM 修复/测量编排归 P6。未发现真实凭据或连接秘密。HMAC ledger 已重新计算，真实唯一覆盖为 1,268/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-126 QxS、Gold 与搜索协议复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查，结论一致。最终 4 项 P4、8 项 P6：QxS 自动后端关键点、Gold96/Gold176 与 Feedback16 的结果、行级证据、数据充分性与声明边界归 P4；后端对照、QxS 结构/能力扫描、Route-B INT8 修复、Gold Cold-start 批量执行及阶段 5 搜索/预算协议编排归 P6。已脱敏远程占位符、环境变量与泛化访问描述不构成 P7。HMAC ledger 已重新计算，真实唯一覆盖为 1,280/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-127 Stage5、Stage7 与边缘证据复审 | 已接受（本地） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；7 项职责或安全边界分歧经第三次只读裁决。最终 5 项 P4、7 项 P6：Stage5 的 AP/cold-start、独立复测与正式结果来源，六臂与 F-Cooper 的正式结果/声明边界，以及边缘测量映射的结果证据归 P4；Stage5 反馈纠正、CPU replay/诊断、后端补齐、边缘测量 runner 修复/表格回填和 Stage7 消融/恢复/三线实验编排归 P6。变量、占位符和省略根的远程/权限/路径叙述不构成 P7，未来公开改写仍须泛化运行细节。HMAC ledger 已重新计算，真实唯一覆盖为 1,292/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-128 Pareto、执行与安全边界复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；8 项职责或安全边界分歧经第三次只读裁决。最终 5 项 P4、3 项 P6、4 项 P7：Pareto 两版、SMBO 收敛、Phase-1 正式记录和 AP/量化闭环证据归 P4；分阶段实验路线、精度路由修复和批量 AP 补齐/导入编排归 P6；4 项历史凭据访问工作流或私有测量来源事实保持 P7，须失效确认、非可执行改写、私有运行信息清除与泄露回归后重审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,304/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-129 数据证据、运行与访问边界复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；3 项职责或安全边界分歧经第三次只读裁决。最终 3 项 P4、3 项 P6、6 项 P7：异常测量策略、冻结训练数据记录和 cost-model 交叉验证证据归 P4；搜索/微调闭环、多工作器 LUT 生产和 replay/smoke/校准/数据生成编排归 P6；6 项私有运行信息、凭据材料或远程身份/访问工作流保持 P7，须非可执行公开改写、私有信息清除、泄露回归及新的独立审查，涉及凭据材料者还须失效确认，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,316/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-130 Stage1、仿真与访问边界复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查；2 项职责分歧经第三次只读裁决。最终 2 项 P4、3 项 P6、7 项 P7：最终实验/论文表格 provenance 与静态标准卷积普查结果归 P4；多代理 LUT 队列、标准卷积 probe/预测器升级和 Stage1 分区/集成合同归 P6；7 项远程身份/访问材料、私有运行或测量来源信息，或外部仿真项目来源、再分发和结果权限未确认的文档保持 P7，须安全改写、来源/许可确认、泄露回归和独立审查后重审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,328/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-131 访问、运行与交接边界复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列身份核验、双独立逐项审查，12 项均为 P7。风险包括远程身份/访问流程、私有运行或测量来源、凭据材料、特权操作，以及外部模型/数据/仿真/结果来源与再分发许可边界；须凭据失效确认（适用时）、非可执行脱敏改写、私有信息清除、来源/许可证确认、泄露回归和新的独立复审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,340/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-132 CoDriving、测量来源与交接边界复审 | 已接受（本地，含 P7 阻塞） | 12 项 `migrate_document` 均完成固定队列核验与双独立逐项审查；q6、q9 的 P7 分歧经第三次只读裁决改为 P4。最终 2 项 P4、10 项 P7：L1 Q-axis 测量/provenance 与历史论文叙事/证据边界归 P4；其余 10 项因凭据/远程访问、私有运行/检查点/数据/测量来源或外部制品许可未确认保持 P7，须失效确认（适用时）、非可执行改写、来源/许可确认、泄露回归和独立复审，本记录不声称已解除。HMAC ledger 已重新计算，真实唯一覆盖为 1,352/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |
| 2026-08-05 | P3-133 TVM、Attention 与 CoDesign 复审 | 已接受（本地，含 P7 阻塞） | 12 项完成双审与第三方裁决；最终 P4=4、P6=7、P7=1。q6 含实际凭据材料，必须轮换/失效确认、移除并完成当前/历史/导出物扫描后重审；其余按第三方裁决归 P4/P6。HMAC ledger 已重新计算，真实唯一覆盖为 1,364/1,551；P3 仍进行中，P4 未启动；未推送、未发布。 |

**P3-140 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方裁决分歧；最终 P4=8、P6=2、P7=1，另有 1 项 `duplicate_or_superseded`。q4 为字节级相同 README 去重，q8/q12 为执行管线，q11 保持 P7；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,448/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-141 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方逐项裁决；最终 P4=6、P6=5、P7=1，无重复项。q6/q7 是不同的执行 fixture，q10 因私有论文 PDF 路径和未确认外部许可保持 P7；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,460/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-142 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方逐项裁决；最终 P4=4、P6=7、P7=1，无重复项。q2–q6、q8、q10 属执行契约，q12 因私有工作树路径及 checkpoint/数据/GPU 资产边界保持 P7；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,472/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-143 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方逐项裁决；最终 P4=2、P6=8、P7=2，无重复项。q2/q12 虽同哈希但缺少公开实现与测试/API职责对比，均保持 P7；q3–q8、q10–q11 为执行契约；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,484/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-144 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方逐项裁决；最终 P4=9、P6=1、P7=2，无重复项。q10/q11 的 RALPH 提示含绝对本机路径和可执行代理命令，保持 P7；q12 实施计划归 P6；其余结果/设计文档归 P4。受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,496/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-145 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审；最终 P4=8、P6=3、P7=1，无重复项。q12 的 Pyramid 架构审计含绝对本机路径及外部 HEAL/checkpoint 边界，保持 P7；受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,508/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-146 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方裁决；最终 P4=3、P6=5、P7=4，无重复项。q2 的私有临时测量日志以及 q7/q11/q12 的外部论文/训练知识来源和许可链未闭合，保持 P7；其余执行工作流归 P6、结果/分析归 P4。受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,520/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-147 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审；最终 P4=9、P6=2、P7=1，无重复项。一项 D-space 文档含私有临时运行路径，保持 P7；survey_task_plan 与协同加速工作流归 P6，其他 survey_raw 文档归 P4。受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,532/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-148 最新进展（2026-08-05）**：12 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方裁决；最终 P4=2、P6=9、P7=1，无重复项。q8 的完整决策清单含绝对私有 Bench2Drive 路径、权重下载源和 checkpoint 边界，保持 P7；其余精度预测/训练/采样/失败分析执行链归 P6，方法总结归 P4。受限证据与 HMAC key 未进入公开树。真实唯一覆盖 **1,544/1,551**，P3 仍进行中，P4 未启动；未推送、未发布。

**P3-149 最新进展（2026-08-05）**：最后 7 项 `migrate_document` 完成 P2/当前 SHA、职责/I-O/调用关系、公开等价物、来源/许可证和安全输出的双独立逐项复审，并由第三方裁决；最终 P4=2、P5=1、P6=2、P7=2，无重复项。P2b 冻结基线已达到 **1,551/1,551** 候选处置覆盖，但全量关闭验证尚未完成，P3 尚未宣布完成，P4 未启动；受限证据与 HMAC key 未进入公开树，未推送、未发布。

**覆盖计算更正及缺口补齐（P3-150–153）**：发现旧选择器将 P3-28/29/30/33 的 48 项 pending `migrate_code` 错计为已覆盖；四个正式缺口批次已完成逐项审阅、第三方裁决（适用时）、受限 ledger 和 acceptance manifest。P3-150 q1 保持 P7；P3-150 q2–q12、P3-151/153 的公开 API/测试职责支持 `rewritten_public`，P3-152 q1 为 P6、其余为公开测试改写。正式 acceptance manifest 现覆盖 **1,551/1,551**，但全量关闭验证仍待完成，P3 尚未宣布完成，P4 未启动；未推送、未发布。
...
**P5 环境契约完成（2026-08-11）**：三份公开 hardware capability YAML、三份 [P5 环境契约](release-manifests/P5_ENVIRONMENT_CONTRACT.md)、合成正负例和离线验证器均已通过本地离线验证。P5 标为**已完成（本地）**；不探测本机 GPU，不运行 CUDA 编译、训练、评测、基准测试或任何外部资产，也不进行网络下载。P6--P8 仍未启动；本记录不授权推送、合并或真实硬件执行。

**P6 H800 自动搜索闭环启动（2026-08-11）**：P6 进入**进行中（本地）**。第一阶段只面向 H800：维护者可通过 Git 忽略的本地配置接入已下载或已生成的资产，并显式启动候选选择、训练/评测、反馈与汇总的真实搜索循环；不得下载资产。公开仓库只保存脱敏资产标签、版本/许可状态和执行摘要，不保存路径、命令、主机信息、原始日志、checkpoint 或逐轮原始结果。Orin 作为后续独立执行线，不以 H800 结果替代。缺失资产、命令失败或指标无效必须失败关闭；本记录不授权推送、合并或发布。

**P6.1 H800/TVM 闭环收口（2026-08-17）**：本地 P6.1 控制器已完成四轮、每轮四候选的真实闭环；每轮均有匹配反馈且无失败终态。此条仅记录脱敏结构性完成事实：原始输入、候选、命令、指标、日志和产物继续保持 Git 忽略，未形成公开结果摘要或论文证据。它不自动启动 P6.2 的真实框架来源闭环、Orin、TensorRT、P7 或 P8，也不授权推送、合并或发布。
...
**P6 RTX4090 迁移收口进展（2026-09-07）**：当前工作分支为 `p6-materializer-training-bridge`，进入本轮收口前的已提交基线为 `ce0835b1a837d224bccab0714ac81b65fafef046`；本轮 README、台账和测试 fixture 收口候选由 Git 历史记录，不在本文件中自引用其最终提交 SHA。双语 README 已新增外部私有 RTX4090 全链路运行说明，明确 `run_p6_h800_search.py` 是历史入口名、RTX4090 contract 是 `configs/execution/p6_rtx4090_search.example.yaml`、derive/normalize 与 provision/preflight 使用不同阶段的 runner-template authority、`GPU_POOL` 是运维侧正整数 GPU 数量且具体设备仅在本次 live admission 后绑定，以及 RTX4090 结果只能作为 sm89 机制验证，不能重标记为 H800 论文数值。本轮测试修复只同步真实 production contract：RTX4090 fixture 必须携带 `measured_rtx4090_tvm` backend scope；并行 source 记录按 group-id 验证，不再依赖并发 JSONL 写入顺序；performance helper 以 schema version 判定 formal profile，避免跨模块 reload 导致类型身份失配。已执行并通过的公开门禁为：失败子集回归 `204 passed`；全仓 `python -m pytest -q` 为 `2813 passed, 1 skipped`；`python -m ruff check .` 通过；`python -m compileall -q framework scripts tools tests` 通过；进入本轮收口前的已提交基线 clean-clone smoke 为 `28 passed`；本轮收口候选提交后的 clean-clone smoke 亦为 `28 passed`。私有 A48 链路已完成 derive/normalize/capability 重建和真实 TVM/AP probe smoke；真实 RTX4090 四卡四轮 controller/verifier 仍等待 GPU admission 满足后运行。当前 GPU0--3 仍有外部利用率，不能把未完成的四卡执行写成已验证。本记录不公开私有路径、设备身份、资产/候选身份、checkpoint、指标、结果、日志或命令参数，也不授权推送、合并、tag、Release 或仓库可见性变更。

P3-139 上一批完成后覆盖为 1,436/1,551（P4=12）。

P3-138 上一批完成后覆盖为 1,424/1,551（P4=12）。

P3-137 上一批完成后覆盖为 1,412/1,551（P4=8、P5=1、P6=1、P7=0、duplicate=2）。

P3-136 上一批完成后覆盖为 1,400/1,551（P4=1、P5=1、P6=8、P7=1、duplicate=1）。

P3-135 上一批完成后覆盖为 1,388/1,551（P4=4、P6=8、P7=0）。
