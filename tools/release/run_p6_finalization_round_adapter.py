#!/usr/bin/env python3
"""Run one P6 post-source finalization round from a generated wrapper."""
...
from framework.stage6.p6_finalization_round_adapter_v1 import (  # noqa: E402
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("measurement_request", type=_absolute_path)
    parser.add_argument("task_state", type=_absolute_path)
    parser.add_argument("actual_feedback", type=_absolute_path)
    parser.add_argument("actual_receipt", type=_absolute_path)
    parser.add_argument("finalization_barrier", type=_absolute_path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        run_finalization_round(
            profile,
            args.measurement_request,
            args.task_state,
            args.actual_feedback,
            args.actual_receipt,
            args.finalization_barrier,
            args.round_output_root,
            SubprocessLeafRunner(),
        )
    except (P6PostSourceAdapterProfileError, P6FinalizationRoundAdapterError):
        sys.stderr.write("history_execution_invalid\n")
        return 1
    except (OSError, TypeError, ValueError):
        sys.stderr.write("history_execution_invalid\n")
        return 1
    sys.stdout.write("finalization_complete\n")
    return 0
