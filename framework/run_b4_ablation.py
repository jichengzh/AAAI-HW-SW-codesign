...
def wilcoxon_signed_rank(a: list[float], b: list[float]) -> dict:
    ...
    out = {"n": len(a), "n_nonzero": len(nz), "mean_diff": float(np.mean(diffs)) if diffs else 0.0}
    if not nz:
        out.update(
            {
                "method": "none",
    ...
    try:
        ...
        res = wilcoxon(a, b, zero_method="wilcox", alternative="two-sided", mode="auto")
        out.update(
            {
...
# ---------------------------------------------------------------------------
def iso_ap70_ratio(
    joint_visited: set, serial_visited: set, apm: APModel, lut: LatencyLUT, grid: list
) -> dict:
        ...
        per_level.append(
            {
                ...
                "joint_lat_threshold": jt,
                "serial_lat_threshold": st,
...
def plot_hv_boxplot(hv: dict, ref_hv: float, path: Path):
    ...
    ax.axhline(
        ref_hv, ls="--", color="green", lw=1, label=f"reference (global P×S) HV = {ref_hv:.3g}"
    )
...
def plot_point_cloud(
    ...
    def pts(visited):
        xs, ys = [], []
        for w, s in visited:
            xs.append(lut.latency(w, s))
            ys.append(apm.ap70(w))
        return xs, ys
    ...
    for k, p in enumerate(pairs):
        ...
        pg_lat = lut.latency(pg, "tuned")
        pg_ap = apm.ap70(pg)
        wg_lat = lut.latency(wg, "tuned")
        wg_ap = apm.ap70(wg)
    ...
    rx = [r["lat_us"] for r in ref_front]
    ry = [r["ap70"] for r in ref_front]
    order = np.argsort(rx)
    ax.plot(
        np.array(rx)[order],
        np.array(ry)[order],
        ...
        color="green",
        lw=1,
        label="reference Pareto (hidden from arms)",
    )
    ...
    ax.set_ylabel("AP70")
    ax.set_title(
        "B4 layer-3 (headline): evaluated point-cloud — A-serial's cloud lacks the P_g region"
    )
...
def _closedloop_report(pairs: list, rep: dict, apm: APModel, lut: LatencyLUT) -> dict:
        ...
        contrast.append(
            {
                ...
                "pg": list(pg),
                "ap70": p["ap70"],
                "wg_tuned_e2e_ms_est": wg_e2e,
                "pg_tuned_e2e_ms_est": pg_e2e,
                "wg_tuned_ds": wg_ds,
                "pg_tuned_ds": pg_ds,
    ...
    return {
        ...
        "caveat": (
            "DS from CoDriving τ_perc curve + H800→Orin linear scaling "
...
def run(n_seeds=12, budget=60, pop=8, n_starts=3, verbose=True, output_root=None, input_root=None):
    ...
    if verbose:
        ...
        print("=" * 80)
        print(
            f"grid ({len(grid)} widths): "
            + ", ".join(seed_grid.get(w, {}).get("label", str(w)) for w in grid)
        )
        print(f"LUT mode={lut.mode} (real={real_lut})  AP mode={apm.mode} (real={real_ap})")
        print(
            f"{'REAL DATA' if not placeholder else 'PLACEHOLDER (seed fallback)'}"
            f"  ref_hv={ref_hv:.4e}  seeds={n_seeds} budget={budget}"
        )
        print(
            f"detected {len(pairs)} W_g/P_g rank-flip pair(s): "
            + "; ".join(
                f"{list(p['wg'])}/{list(p['pg'])}@AP{p['ap70']}({p['iso_ap_latency_ratio']}×)"
                for p in pairs
            )
            + "\n"
        )
    ...
    if grid:
        _add_start(grid[-1])
        _add_start(grid[0])  # widest, narrowest
        for p in pairs:
            _add_start(p["wg"])
            _add_start(p["pg"])
    ...
    for sd in range(n_seeds):
        ...
        for sw in start_widths:
            rss = run_serial(
                lut, apm, grid, budget, random.Random(sd), hv_ref, pop, start_width=sw, **cl_kw
            )
        ...
        per_pair.append(
            {
                ...
                "wg_tuned_us": p["wg_tuned_us"],
                "pg_tuned_us": p["pg_tuned_us"],
                "wg_tuned_ratio": p["wg_tuned_ratio"],
                "pg_tuned_ratio": p["pg_tuned_ratio"],
                ...
                "n_starts_drop_pg": sum(pg_dropped),
                "n_starts": len(pg_dropped),
            }
        )
    all_pass = bool(pairs) and all(
        pp["joint_reaches_pg_tuned"]
        and not pp["serial_reaches_pg_tuned"]
        and pp["serial_drops_pg_all_starts"]
        for pp in per_pair
    )
    structural = {
        "n_pairs": len(pairs),
    ...
    results = {
        "meta": {
            ...
            "reference_pareto": [
                {
                    "label": seed_grid.get(r["width"], {}).get("label", str(r["width"])),
                    "sched": r["sched"],
                    "ap70": r["ap70"],
                    "lat_us": r["lat_us"],
                }
            ...
            "greedy_order_note": (
                "P->S is the only non-degenerate order: S is a "
        ...
        "visited": {
            "A-joint": sorted(
                [seed_grid.get(w, {}).get("label", str(w)), s] for (w, s) in joint_visited
            ),
            "A-noS": sorted(
                [seed_grid.get(w, {}).get("label", str(w)), s] for (w, s) in nos_visited
            ),
            "A-serial": sorted(
                [seed_grid.get(w, {}).get("label", str(w)), s] for (w, s) in serial_visited
            ),
        },
        "figures": {
...
def _print_report(r: dict, ref_hv: float):
    ...
    for pp in s["per_pair"]:
        print(
            f"  pair W_g={pp['wg']} / P_g={pp['pg']} @ iso-AP70={pp['ap70']}: "
    ...
    for pp in s["per_pair"]:
        print(
            f"  {pp['wg']}→{pp['pg']}: joint reaches P_g/tuned={pp['joint_reaches_pg_tuned']} "
    ...
    print(
        f"  ({s['n_pairs']} pair(s), {s['n_starts_per_seed']} starts/seed, "
    ...
    print(
        ...
        + (
            ...
            if ok
            else "see per-pair fields above."
        )
    )
