...
def plot_hv_boxplot(hv: dict, ref_hv: float, path: Path):
    ...
    ax.axhline(
        ref_hv, ls="--", color="green", lw=1, label=f"reference (global P×Q×S) HV = {ref_hv:.3g}"
    )
...
def plot_pareto_int8(joint_visited, serial_visited, apm, lut, qlut, q_pairs, hv_ref, path: Path):
    ...
    ax.scatter(
        ...
        color="#2c7fb8",
        alpha=0.85,
        label="A-joint INT8 (reached)",
    )
    ax.scatter(
        sp["fp16"][0], sp["fp16"][1], s=42, marker="x", color="#d95f0e", label="A-serial fp16"
    )
    ax.scatter(
        sp["int8"][0],
        sp["int8"][1],
        ...
        color="#d95f0e",
        edgecolors="k",
        label="A-serial INT8 (only aligned locks)",
    )
    ...
    for k, p in enumerate(q_pairs):
        ...
        ax.annotate(
            f"P_g={list(pg)} s0=64\nINT8 buildable\nW_g={list(wg)} s0=48 INT8 UNBUILDABLE",
            (pg_lat, pg_ap),
    ...
    ax.set_ylabel("AP70")
    ax.set_title(
        "P×Q×S: A-joint reaches the INT8 P_g frontier; "
...
def run(
    ...
    if manifest:
        ...
        if verbose:
            g = b["gating_knob"]
            print(
                f"  [bridge] manifest={Path(manifest).name} key_scale={key_scale} "
                f"int8_buildable_align={b['bridge_int8_align']} "
                f"gating={g.search_group_id if g else None}"
            )
            print("  [dispatch] 逐-knob 耦合分流 (stage1 耦合分数→stage2 内环预算):")
            for d in b["dispatch_plan"]:
                print(
                    f"    {d['knob']:18s} score={d['coupling_score']:.3f} "
    ...
    if verbose:
        ...
        print(
            "INT8 UNBUILDABLE (s0 misaligned): "
    ...
    for sd in range(n_seeds):
        ...
        hv["A-joint-PQS"].append(rj.final_hv)
        conv["A-joint-PQS"].append(rj.cost.conv_log)
        hv["A-noS-PQS"].append(rn.final_hv)
        conv["A-noS-PQS"].append(rn.cost.conv_log)
        hv["A-serial-PQS"].append(rs.final_hv)
        conv["A-serial-PQS"].append(rs.cost.conv_log)
        ...
        per_pair.append(
            {
                ...
                "pg": list(pg),
                "ap70": p["ap70"],
                "wg_in_per_g": qlut.in_per_g(wg),
                "pg_in_per_g": qlut.in_per_g(pg),
    ...
    cat_pass = bool(ps_pairs) and all(
        (not pp["wg_int8_buildable"])
        and pp["pg_int8_buildable"]
        and pp["joint_reaches_pg_int8"]
        and not pp["serial_reaches_pg_int8"]
        for pp in per_pair
    )
    ...
    serial_misaligned_int8_on_pareto = any(
        r["quant"] == "int8" and not qlut.can_build_int8(tuple(r["width"])) for r in serial_pareto
    )
    ...
    n_joint_int8_widths = len(
        {w for (w, s, q) in joint_visited if q == "int8" and qlut.can_build_int8(w)}
    )
    n_serial_int8_widths = len(
        {w for (w, s, q) in serial_visited if q == "int8" and qlut.can_build_int8(w)}
    )
    ...
    results = {
        "meta": {
            ...
            "caveat": (
                "INT8 latency is a UNIFORM per-width stage0-speedup proxy; "
        ...
        "q_rank_flip_pairs": [
            {
                ...
                "pg": list(p["pg"]),
                "ap70": p["ap70"],
                "wg_int8_us": p["wg_int8_us"],
                "pg_int8_us": p["pg_int8_us"],
                "q_rank_flip_ratio": p["q_rank_flip_ratio"],
                "source": p["source"],
            }
            for p in q_pairs
        ],
        ...
        "structural_q_claim": {
            ...
            "interpretation": (
                ...
                if cat_pass
                else "CHECK per_pair fields."
            ),
        },
        "visited_int8": {
            "A-joint-PQS": sorted(
                [seed_grid.get(w, {}).get("label", str(w)), s]
                for (w, s, q) in joint_visited
                if q == "int8" and qlut.can_build_int8(w)
            ),
            "A-serial-PQS": sorted(
                [seed_grid.get(w, {}).get("label", str(w)), s]
                for (w, s, q) in serial_visited
                if q == "int8" and qlut.can_build_int8(w)
            ),
        },
...
def _print_report(r: dict):
    ...
    for a in ("A-joint-PQS", "A-serial-PQS", "A-noS-PQS"):
        ...
        print(
            f"  {a:14s} mean={s['mean']:.4e} ({s['pct_of_joint']:5.1f}% of joint) "
    ...
    for pp in sc["per_pair"]:
        print(
            f"  W_g={pp['wg']}(in/g={pp['wg_in_per_g']},int8={pp['wg_int8_buildable']}) "
