...
用法:
  PYTHONPATH=. python -m framework.stage1.run_scan --model all --device cpu
...
def get_scan_adapter(model: str):
    if model in REGISTRY:
        return get_adapter(model)
    if model in AUTO_REGISTRY:
        return get_auto_adapter(model)
    raise KeyError(f"unknown model '{model}', choices={ALL_MODELS}")
...
def run_one(model: str, hw: HwCapability, device: str, out_dir: Path,
    ...
    try:
        adapter = get_scan_adapter(model)
