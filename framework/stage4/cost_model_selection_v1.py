...
def derive_seed(
    ...
    *,
    ...
    """Return the frozen Appendix seed schedule for one nested-CV operation."""
    if purpose == "inner":
        return seed + outer_index + 1
    if purpose == "outer_fit":
        return seed + 1000 + outer_index
    if purpose == "candidate" and candidate_index is not None and inner_index is not None:
        return seed + 100 * outer_index + 10 * candidate_index + inner_index
    raise ValueError("candidate seed derivation requires candidate_index and inner_index")


def _require_unique_records(
    records: Sequence[Mapping[str, Any]], *, identity: str, context: str
) -> None:
    seen: set[str] = set()
    ...
    *,
    ...
    for target in TARGETS:
        ...
        for outer_index, outer in enumerate(outer_folds):
            ...
            # Appendix A: the inner-fold seed is derived only from the outer fold.
            inner = grouped_folds(
                inner_rows,
                n_splits=min(inner_splits, len({row["group_id"] for row in inner_rows})),
                seed=derive_seed(seed, outer_index=outer_index, purpose="inner"),
            )
            ...
            for candidate_index, name in enumerate(candidate_names):
                ...
                for inner_index, fold in enumerate(inner):
                    ...
                    prediction = _fit_predict_candidate(
                        ...
                        fit_rows,
                        validation_rows,
                        # Appendix A: candidate/inner seeds are disjoint and deterministic.
                        seed=derive_seed(
                            seed,
            ...
            prediction = _fit_predict_candidate(
                ...
                # Appendix A: final outer-fold fits use a separate seed range.
                seed=derive_seed(seed, outer_index=outer_index, purpose="outer_fit"),
            )
