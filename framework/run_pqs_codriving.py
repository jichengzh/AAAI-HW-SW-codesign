...
def run(
    ...
    if verbose:
        ...
        print(
            f"detect_wg_pg_pairs -> {len(ps_pairs)} rank-flip pairs "
    ...
    results = {
        ...
        "verdict": {
            ...
            "interpretation": (
                ...
                "per-channel granularity / int4 / batch to hunt a higher-dim trap."
                if separable
                else "UNEXPECTED non-separable signal at low dim — inspect HV/pairs."
            ),
        },
    }
    ...
    if verbose:
        ...
        print(f"\nWilcoxon joint vs serial: p={wilcox['p_value']} method={wilcox['method']}")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="CoDriving P×Q×S separability smoke")
