...
PUBLIC_SELECTION_FIELDS = frozenset(
    {
        "candidate_id",
        "width",
        "q_mode",
...
def select_hardware_blind_batch(
    ...
    columns = {field: [float(row[field]) for row in rows] for field in BLIND_FIELDS}
    scored = []
    for row in rows:
        score = (
    ...
    return [
        {key: value for key, value in row.items() if key in PUBLIC_SELECTION_FIELDS}
