...
def _mapping(value: Any) -> Mapping[str, Any]:
    """Treat malformed nested values as absent so validation fails closed."""
    return value if isinstance(value, Mapping) else {}


def build_stage6_manifest() -> dict[str, Any]:
    """Return a fresh Stage6 contract without binding local evidence locations."""
    common = {
    ...
    arms = [
        ...
        },
        {
            ...
            "joint_evidence": {
                "tvm": {"availability": "external", "required_artifact": "stage5_feedback_closure"},
                "trt": {"availability": "external", "required_artifact": "stage5_feedback_closure"},
            },
            "joint_summary_evidence": {
                "availability": "external",
                "required_artifact": "stage5_feedback_summary",
            },
            "joint_online_rows_evidence": {
                "availability": "external",
                "required_artifact": "stage5_online_rows",
            },
...
def validate_stage6_manifest(manifest: Any) -> dict[str, Any]:
    ...
    dispatch = _mapping(common.get("precision_dispatch_contract"))
    if (
    ...
    forward = _mapping(by_id.get("compress_then_tune"))
    ...
    reverse = _mapping(by_id.get("tune_then_compress"))
    reverse_ok = all(
        (
    ...
    schedule = _mapping(by_id.get("schedule_only"))
    backend_contracts = _mapping(schedule.get("backend_contracts"))
    if set(backend_contracts) != {"tvm", "trt"}:
        failures.append("schedule_only_backend_contract_missing")
    elif not all(isinstance(contract, Mapping) for contract in backend_contracts.values()) or any(
    ...
    if evidence_is_valid:
        evidence_is_valid = all(
            ...
            and item.get("availability") == "external"
            and bool(item.get("required_artifact"))
            for item in evidence.values()
        )
    if not evidence_is_valid:
        failures.append("joint_evidence_binding_missing")
    summary = joint.get("joint_summary_evidence") or {}
    rows = joint.get("joint_online_rows_evidence") or {}
    if (
        not isinstance(summary, Mapping)
        or summary.get("availability") != "external"
        or not summary.get("required_artifact")
        or not isinstance(rows, Mapping)
        or rows.get("availability") != "external"
        or not rows.get("required_artifact")
    ):
