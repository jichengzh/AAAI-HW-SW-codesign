...
def compiler_fingerprint(runtime_identity: Mapping[str, Any]) -> str:
    ...
    if any(
        ...
        for key in ("support_root_sha256", "tvm_site_sha256", "nvlibs_file_sha256")
    ):
        raise P6CapabilityContextError()
    ...
    if any(
        ...
        for key in ("tvm_version", "python_version", "target", "tvm_arch")
    ):
        raise P6CapabilityContextError()
...
def _validated_evidence(
    ...
    *,
    ...
    evidence["runtime_identity"] = _validated_runtime(
...
def _rebuilt_active_profile(
    evidence: Mapping[str, Any], features: Mapping[str, float | int]
) -> dict[str, Any]:
...
def build_rtx_capability_context(
    *,
    ...
    historical = _validated_historical_profiles(historical_profiles)
    ...
    active = _rebuilt_active_profile(measured, features)
    payload = _context_payload(historical, active, measured)
    return ValidatedP6CapabilityContext(historical, active, measured, _sha(payload))
...
def capability_context_to_mapping(
    ...
    payload = _context_payload(
        context.historical_profiles, context.active_profile, context.evidence
    )
...
def validate_rtx_capability_context(
    ...
    *,
    ...
    rebuilt = build_rtx_capability_context(
        historical_profiles=raw["historical_profiles"],
