...
from framework.stage6.p6_source_wrapper_template_v1 import (
...
PROFILE_SCHEMA_VERSION_V2 = "p6_private_source_wrapper_profile_v2"
...
PROFILE_V2_KEYS = PROFILE_KEYS | {"project_python"}
...
PROJECT_PYTHON_ASSIGNMENT = re.compile(
    r"\APY=\$\{PY:-(?P<path>/[^${}\s]+)\}\Z"
)
...
def extract_project_python_from_source(implementation: Path) -> Path:
    """Extract one literal historical PY default as a canonical executable."""
    if (
        not isinstance(implementation, Path)
        or not implementation.is_absolute()
        or _contains_symlink_component(implementation)
    ):
        _invalid()
    try:
    ...
    if len(assignments) != 1:
        _invalid()
    matched = PROJECT_PYTHON_ASSIGNMENT.fullmatch(assignments[0])
    if matched is None:
        _invalid()
    try:
        resolved = Path(matched.group("path")).resolve(strict=True)
    except OSError as error:
        raise P6SourceWrapperProfileError() from error
    return _validated_project_python(str(resolved))
...
def validate_self_contained_source_wrapper(
    ...
    *,
    ...
    if (
        plan.project_python is None
        or tuple(validated_template.stage_argv) != EXPECTED_EXECUTION_STAGES
    ):
        _invalid()
...
def _build_render_plan(
    ...
    *,
    ...
    elif schema_version == PROFILE_SCHEMA_VERSION_V2:
        expected_keys = PROFILE_V2_KEYS
    else:
        _invalid()
    if set(profile) != expected_keys or profile.get("wrapper_kind") != WRAPPER_KIND:
        _invalid()
    project_python = (
        _validated_project_python(profile["project_python"])
...
def _validated_project_python(raw: object) -> Path:
    ...
    try:
        resolved = path.resolve(strict=True)
        python_launcher = resolved.parent / "python"
