...
AP_MODEL_JSON = next((p for p in _AP_MODEL_CANDIDATES if p.exists()), _AP_MODEL_CANDIDATES[0])
...
class LatencyLUT:
    ...
    def latency(self, width: Width, sched: str) -> float:
        ...
        if self.per_stage:
            return sum(self._interp(self.per_stage[k], width[k], sched) for k in range(len(width)))
...
class APModel:
    ...
    def __init__(
        self, ap_path: Path = AP_MODEL_JSON, seed_path: Path = SEED_GRID_JSON, key_scale: int = 1
    ):
        self.key_scale = int(key_scale)  # >1 → bridge-native cur_width keys
...
class CostModel:
    ...
    def evaluate(self, width: Width, sched: str) -> tuple[tuple[float, float], dict]:
        ...
        self.n_unique += 1
        rec = {"n": self.n_unique, "width": width, "sched": sched, "ap70": ap, "lat_us": lat}
        if self.use_energy:
...
def hypervolume_2d(points_min: list[tuple[float, float]], ref: tuple[float, float]) -> float:
...
def nsga2(
    ...
    rng: random.Random,
    max_gens: int = 200,
    forced_init: Optional[list[tuple[int, ...]]] = None,
) -> None:
...
def run_joint(
    ...
    cost = CostModel(
        ...
        hv_ref,
        use_energy,
        use_closedloop=use_closedloop,
        cl_beta=cl_beta,
        cl_scale_tier=cl_scale_tier,
    )

    def decode(g):
        return width_grid[g[0]], SCHEDULES[g[1]]
    ...
    return ArmResult("A-joint", cost, pareto, visited, hypervolume_2d(cost.archive, hv_ref))
...
def run_noS(
    ...
    def decode(g):
        return width_grid[g[0]], "default"

    nsga2(cost, decode, [len(width_grid)], pop_size, budget, rng)
    pareto = _front_records(cost.eval_log)
    visited = [(r["width"], r["sched"]) for r in cost.eval_log]
    return ArmResult("A-noS", cost, pareto, visited, hypervolume_2d(cost.archive, hv_ref))
...
def run_serial(
    ...
    if topk:
        ...
        extra = sorted(
            (r for r in default_recs if r["width"] not in locked), key=lambda r: r["lat_us"]
        )
        for r in extra[:topk]:
    ...
    if locked:
        ...
        def decode2(g):
            return locked[g[0]], SCHEDULES[g[1]]
...
def reference_pareto(width_grid, lut, apm, hv_ref):
    ...
    for w in width_grid:
        for s in SCHEDULES:
            recs.append({"width": w, "sched": s, "ap70": apm.ap70(w), "lat_us": lut.latency(w, s)})
...
def detect_wg_pg_pairs(
    width_grid: list[Width], lut: LatencyLUT, apm: APModel, ap_tol: float = 1e-4
) -> list[dict]:
...
def build_from_manifest(
    ...
    cliff = [k for k in spec.knobs if set(k.legal_widths()) - set(k.buildable_int8_widths())]
    ...
    return {
        "spec": spec,
        "key_scale": key_scale,
        "bridge_int8_align": bridge_align,
        "lut": lut,
        "apm": apm,
        "qlut": qlut,
        "gating_knob": gating,
        "dispatch_plan": dispatch,
        "joint_knobs": [d["knob"] for d in dispatch if d["dispatch"] == "joint"],
    }
...
def run_smoke(seeds=(0, 1, 2, 3, 4), budget=40, pop_size=8, verbose=True):
    ...
    print("=" * 84)
    print(
        f"LUT mode={lut.mode}  AP mode={apm.mode}  hv_ref(lat,-ap)={hv_ref[0]:.0f},{hv_ref[1]:.3f}"
    )
    print("Reference (global P×S) Pareto [HIDDEN from arms, scoring only]:")
    ...
    for sd in seeds:
        if verbose:
            print(f"--- seed {sd} ---")
        for runner, name in ((run_joint, "A-joint"), (run_noS, "A-noS"), (run_serial, "A-serial")):
            ...
            if verbose:
                ...
                if res.locked_widths is not None:
                    extra = "  locked=" + ",".join(_label(w, seed_grid) for w in res.locked_widths)
                print(
                    f"  {name:9s} HV={res.final_hv:.4e}  evals={res.cost.budget_used():2d}{extra}"
                )
    ...
    for name in ("A-joint", "A-serial", "A-noS"):
        ...
        print(
            f"  {name:9s} mean HV={mean:.4e}  ({mean / ref_hv * 100:5.1f}% of ref)  "
    ...
    for name in ("A-joint", "A-noS", "A-serial"):
        ...
        print(f"  {name:9s} ({len(pts):2d}): " + ", ".join(f"{lab}/{s}" for lab, s in pts))
    ...
    print(
        f"  A-serial visits (pad64, tuned) : {serial_has}   "
    ...
    print(
        "\n  RESULT: "
        + (
            "PASS — A-serial structurally misses (P_g=pad64, tuned); A-joint reaches it."
            if ok
            else "FAIL — see above."
        )
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--q-mode", action="store_true", help="Run Q-extended P×Q×S smoke test")
    args = ap.parse_args()
    if args.q_mode:
...
class QLookup:
    ...
    def __init__(
        ...
        self.bridge_int8_align = None if bridge_int8_align is None else int(bridge_int8_align)
...
class CostModelPQS:
    ...
    def evaluate(
        self, width: Width, sched: str, quant: str = "fp16"
        ...
        if quant == "fp16":
        ...
        elif self.qlut.enforce_int8_buildable and not self.qlut.can_build_int8(width):
            ...
            if self.qlut.has_direct_h800(width, sched):
                ...
                lat_source = (
                    f"H800_TVM_fp16/{self.qlut.uniform_int8_speedup}(measured_stage0_speedup_proxy)"
                )
...
def run_joint_pqs(lut, apm, qlut, width_grid, budget, rng, hv_ref, pop_size):
    ...
    nsga2_pqs(
        cost, decode, [len(width_grid), len(SCHEDULES), len(QUANT_MODES)], pop_size, budget, rng
    )
    pareto = _front_records_pqs(cost.eval_log)
    visited = [(r["width"], r["sched"], r["quant"]) for r in cost.eval_log]
    return ArmResult("A-joint-PQS", cost, pareto, visited, hypervolume_2d(cost.archive, hv_ref))
...
def run_noS_pqs(lut, apm, qlut, width_grid, budget, rng, hv_ref, pop_size):
    ...
    nsga2_pqs(cost, decode, [len(width_grid), len(QUANT_MODES)], pop_size, budget, rng)
    pareto = _front_records_pqs(cost.eval_log)
    visited = [(r["width"], r["sched"], r["quant"]) for r in cost.eval_log]
    return ArmResult("A-noS-PQS", cost, pareto, visited, hypervolume_2d(cost.archive, hv_ref))
...
def run_serial_pqs(
    ...
    default_recs = [r for r in cost.eval_log if r["sched"] == "default" and r["quant"] == "fp16"]
    ...
    visited = [(r["width"], r["sched"], r["quant"]) for r in cost.eval_log]
    final_hv = hypervolume_2d([(r["lat_us"], -r["ap70"]) for r in deliverable], hv_ref)
    arm = ArmResult("A-serial-PQS", cost, pareto, visited, final_hv, locked_widths=locked)
    return arm
...
def detect_q_rank_flip_pairs(
    width_grid: list[Width],
    lut: LatencyLUT,
    apm: APModel,
    qlut: QLookup,
    ap_tol: float = 1e-4,
    ...
    for i in range(len(g)):
        for j in range(i + 1, len(g)):
            ...
            is_measured = qlut.has_direct_h800(wg_fp16, sched) and qlut.has_direct_h800(
                pg_fp16, sched
            )
            pairs.append(
                {
                    ...
                    "q_rank_flip_ratio": round(
                        ...
                        / qlut.int8_lat(lut.latency(pg_fp16, sched), pg_fp16, sched=sched),
                        3,
                    ),
...
def candidate_widths_pqs(lut: LatencyLUT, apm: APModel, qlut: QLookup) -> list[Width]:
...
def run_smoke_pqs(seeds=(0, 1, 2, 3, 4), budget=60, pop_size=8, verbose=True):
    ...
    if pending:
        print(
            f"[WARNING] {len(pending)} widths have pending Q measurements "
            f"(speedup=1.0 fallback): {pending}"
        )
        print("  → Run hw-optimizer to measure: pad64 [64,96,192]")
    ...
    for p in s_pairs:
        print(f"  S-flip: W_g={p['wg']} vs P_g={p['pg']} ratio={p['iso_ap_latency_ratio']}x")
    ...
    for p in q_pairs:
        ...
        print(
            f"  Q-flip: W_g={p['wg']}(INT8={p['wg_int8_speedup']:.3f}x) "
    ...
    pad64 = (64, 96, 192)
    j_int8_pg = any(
        r["quant"] == "int8" and r["width"] == pad64
        for res in summary["A-joint-PQS"]
        for r in res.cost.eval_log
    )
    s_int8_pg = any(
        r["quant"] == "int8" and r["width"] == pad64
        for res in summary["A-serial-PQS"]
        for r in res.cost.eval_log
    )
    s_q_never = all(
        r["quant"] == "fp16" for res in summary["A-serial-PQS"] for r in res.cost.eval_log
    )
    print("\n" + "=" * 84)
    ...
    print(
        "\n  RESULT: "
        + (
            "PASS — A-serial structurally excludes Q-axis; A-joint-PQS explores INT8."
            if ok
            else "FAIL — see above."
        )
    )
...
__all__ = [
    ...
    "run_serial_pqs",
    "nsga2_pqs",
    "detect_q_rank_flip_pairs",
    "candidate_widths_pqs",
    "compute_hv_ref_pqs",
    "SMOKE_WIDTHS_PQS",
    "Q_LUT_JSON",
    "run_smoke_pqs",
