...
def _real_ap(apm: APModel, w) -> dict:
    ...
    return {
        "width": list(w),
        "real_measured": bool(exact),
        "source": "apm.exact (stage_a / DepGraph finetune)" if exact else "INTERPOLATED",
        "ap70": apm.ap70(w),
    }
...
def main(output_root: str | Path | None = None, input_root: str | Path | None = None) -> None:
    ...
    grec = [
        {"width": w, "sched": "tuned", "ap70": apm.ap70(w), "lat_us": lut.latency(w, "tuned")}
        for w in grid
    ]
    ...
    for p in pairs:
        ...
        kind = (
            "shipped_codesign_win"
            if (pg_on_global and joint_ships_pg and serial_ships_wg)
            else "mechanism_only"
        )
        pair_story.append(
            {
    ...
    out = {
        "summary": {
            ...
            "mechanism_only_ratios": [
                ps["iso_ap70_latency_ratio"] for ps in pair_story if ps["kind"] == "mechanism_only"
            ],
            "n_pairs": len(pairs),
            "n_shipped_wins": len(wins),
            "note": (
                "Headline = shipped co-design wins (P_g on global Pareto, "
