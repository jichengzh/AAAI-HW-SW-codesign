...
def config_driving_score(
    ...
    if config_label not in _CONFIG_GRID:
        raise ValueError(f"Unknown config {config_label!r}. Available: {sorted(_CONFIG_GRID)}")
...
def b4_config_table(
    ...
    return [
        config_driving_score(c, schedule=schedule, scale_tier=scale_tier, beta=beta)
        for c in configs
    ]
...
def _self_check() -> None:
    ...
    for row in b4_config_table():
        print(
            f"{row['config_label']:>8} {row['h800_backbone_us']:>8.0f} "
    ...
    for label, real_e2e in [("base", 218.83), ("p75", 107.52)]:
        ...
        print(
            f"  {label}: estimated={est:.1f}ms, real={real_str}ms "
