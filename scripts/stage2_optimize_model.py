...
from framework.stage2.contracts import Stage2Input, build_stage2_output  # noqa: E402
...
def _write_json(path: str | Path, data: dict) -> None:
    ...
    if out == Path("/"):
        raise ValueError("output path must not be the filesystem root")
    out.parent.mkdir(parents=True, exist_ok=True)
    if out.is_symlink() or (out.exists() and not out.is_file()):
        raise ValueError(f"refusing to write non-file output: {out}")
...
def main() -> int:
    ...
    if args.out_md:
        requested_md = Path(args.out_md).expanduser()
        lexical_md = requested_md.absolute()
        out_md = requested_md.resolve(strict=False)
        if lexical_md != out_md:
            raise ValueError("output path must not traverse a symlink or parent escape")
        if out_md == Path("/"):
            raise ValueError("output path must not be the filesystem root")
        out_md.parent.mkdir(parents=True, exist_ok=True)
        if out_md.is_symlink() or (out_md.exists() and not out_md.is_file()):
            raise ValueError(f"refusing to write non-file output: {out_md}")
        rendered_md = _render_md(data)
        if out_md.exists() and out_md.read_text(encoding="utf-8") != rendered_md:
            raise ValueError(f"refusing to overwrite existing output: {out_md}")
