...
from tests.stage1.test_structural_axes import (
    _built_selector_inputs,
    _resign_inputs,
    _scenario_evidence,
    _selector_context,
    _selector_group_manifest,
    _selector_groups,
    _selector_scenario,
)
...
def test_formal_max_rate_comes_from_scenario_policy_not_depgraph_diagnostic() -> None:
    ...
    groups = [{**_selector_groups()[0], "max_rate": 0.5}]
...
def test_formal_axis_fails_closed_without_scenario_max_rate_policy() -> None:
    ...
    with pytest.raises(ValueError, match="formal max_rate policy"):
        build_structural_axis_inputs(
            ...
            scenario=scenario,
            group_manifest=_selector_group_manifest(scenario=scenario),
        )
...
def test_legal_widths_reject_malformed_max_rate(numerator: object) -> None:
    scenario = _scenario_evidence(_selector_scenario())
...
def test_build_inputs_rejects_binding_without_depgraph_group() -> None:
    groups = _selector_groups(root_layer="somewhere.else")
